...
public final class EditableCells {
    ...
    /**
     * Factoría de celda con TextField para fechas. Estilo CONTENDO
     * <code>&lt;input type="date"&gt;</code>: el usuario teclea
     * libremente, al perder foco / Tab / Enter parsea con
     * {@link #parseFlexibleDate} y guarda como {@link LocalDate};
     * al renderizar muestra en ISO {@code yyyy-MM-dd}.
     *
     * <p>Diseñado para sustituir {@link #datePicker()} cuando el
     * comportamiento de JavaFX DatePicker (popup, lifecycle, commit
     * de blur con converter locale) causa problemas en TableViews
     * con muchas filas dinámicas. Versión simple y robusta sin popup.
     *
     * <p>Si el usuario teclea algo que no parsea, la celda mantiene
     * el texto rojo y NO commitea — el valor anterior se conserva.
     */
    public static <S> Callback<TableColumn<S, LocalDate>, TableCell<S, LocalDate>>
            flexibleDateTextField() {
        return col -> new FlexibleDateTextCell<>();
    }

    private static class FlexibleDateTextCell<S> extends TableCell<S, LocalDate> {
        ...
        public void startEdit() {
            ...
            LocalDate v = getItem();
            field.setText(v == null ? "" : ISO.format(v));
            field.getStyleClass().remove("flexible-date-error");
            setText(null);
            setGraphic(field);
            field.selectAll();
            field.requestFocus();
        ...
        protected void updateItem(LocalDate v, boolean empty) {
            ...
            if (isEditing()) {
                if (field != null) field.setText(v == null ? "" : ISO.format(v));
                setText(null);
                setGraphic(field);
            } else {
                setText(v == null ? "" : ISO.format(v));
        ...
        private void buildField() {
            field = new TextField();
            // Commit al perder foco: parsea + actualiza si OK.
            field.focusedProperty().addListener((obs, was, now) -> {
                if (was && !now && isEditing()) {
                    tryCommit();
                }
            });
            field.setOnAction(ev -> tryCommit());
        ...
        private void tryCommit() {
            ...
            if (typed == null || typed.isBlank()) {
                // Vaciar la fecha está permitido (campo opcional).
                commitEdit(null);
                return;
            }
            LocalDate parsed = parseFlexibleDate(typed);
            if (parsed != null) {
                ...
                // Marcar visualmente que no parsea; mantener edición.
                if (!field.getStyleClass().contains("flexible-date-error")) {
                    field.getStyleClass().add("flexible-date-error");
                }
                field.setStyle("-fx-border-color: #c0392b; -fx-border-width: 2;");
            }
        }
    }
