...
- ✅ Slice C3: módulo Configuración MVP — V9 + Jasypt + 3 controllers `/api/settings/*` con `@RequiresRole`/`@RequiresModule` + UI con TabPane (Empresa/Email SMTP/Módulos) + sticky footer + batched save de módulos + A4 sidebar dinámico (`/api/modules-catalog/active`). ⚠️ i18n pendiente (labels y hints hardcodeados).
...
La app se diseñó bilingüe desde C1 (botón EN/ES en el header → `language` field + helper `t(key)` en `BenjagestUiApplication`). Desde C3 y especialmente en F2/F3/F4/F5/F5+ **se han hardcodeado strings en español** sin pasar por `t()`. Hay que recorrer cada pantalla, mover los literales a claves, y traducirlos al inglés. Lista:

- ⬜ **F2/F3/F5 Facturación shell** — sub-tabs (Dashboard/Facturas/Configuración), header "Facturación", botones "Nueva factura", "Aplicar filtros", "Limpiar", filtros "(todos)", placeholder tabla, hints. [pendiente].
- ⬜ **F4 Editor de factura** — todos los strings introducidos en el rediseño estilo CONTENDO: cabecera "Cabecera de la factura"/"Cliente *"/"Fecha emisión"/"Fecha vencimiento"/"Tipo"/"Factura normal · serie automática", card "Conceptos / Líneas", botones "Añadir/Quitar línea", "Notas / Observaciones", totales "Base imponible/Cuota IVA total/Retención IRPF/TOTAL FACTURA", footer "Cancelar / Guardar borrador / Validar y emitir", badge "PRÓXIMO Nº AL VALIDAR", panel "prerequisitePanel" sin clientes/series, alertas de validación ("Falta cliente", "Sin líneas", "Línea incompleta", "No se pudo validar/guardar"). [pendiente].
- ⬜ **F5+ Configuración facturación** — secciones "VeriFactu"/"Series de numeración"/"Migración desde otro programa"/"Textos legales en la factura", labels de form, hints, columnas de tablas series, botones "Definir mi serie de facturas"/"Editar mi serie", textos del diálogo `showSeriesEditor` (incluido el aviso de autobloqueo y de serie reservada del sistema), alertas migración. [pendiente].
- ⬜ **F4 Listado de facturas** — columnas (Numero/Cliente/Fecha/Vencimiento/Estado/Cobro/Total), texto "(borrador)", botones de acción "Validar y emitir / Eliminar borrador / Generar PDF", confirmaciones, alerta "Pendiente · slice F4b". [pendiente].
- ⬜ **Command Palette + atajos** (slice de hoy) — título "BENJAGEST · Comandos", placeholder "Buscar acción…", lista de acciones ("Inicio · dashboard", "Clientes", "Facturación", "Nueva factura", "Configuración", "Agenda", "Compras", "Fiscal", "Laboral", "Informes"). [pendiente].
- ⬜ **C3 Configuración (Empresa/Email/Módulos/Auditoría)** — auditar también; muchos labels y hints fueron hardcodeados en español. [pendiente].
- ⬜ **Diálogos genéricos** — `showError(title, message)`, alerts de confirmación de borrado/anulación, mensajes de "No se pudo …". [pendiente].
...
- 🔵 **Command Palette `Ctrl+K`** — buscador global rápido. Implementado el palette + atajos (Ctrl+K abrir, Ctrl+N nueva factura, Ctrl+F facturación, Ctrl+H inicio, F5 refresh, mouse BACK/FORWARD) en sesión 2026-06-02. ⚠️ i18n pendiente (labels y acciones hardcoded en ES). Pendiente: ampliar lista de acciones según vayan saliendo módulos. [§2.2 `gap-analysis-config-ui`](gap-analysis-config-ui.md).
