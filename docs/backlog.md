...
> **Última actualización: 2026-07-16 (mañana) — ✅ UPD-3 + UPD-3b MERGEADOS a develop. 130 VERIFICADO e2e. MSI `BENJAGEST-0.1.43.msi` construido y verificado, **SIN PUBLICAR RELEASE A PROPÓSITO**. Suite 142 verde. Detalle en la sesión 2026-07-16 justo abajo.**
>
> ## 🚀 PROMPT PARA LA PRÓXIMA SESIÓN (copiar y pegar)
>
> *"Lee la cabecera de docs/backlog.md y la sección de la SESIÓN 2026-07-16. El MSI **0.1.43** está construido en `dist\msi\BENJAGEST-0.1.43.msi` y verificado por extracción; **falta que Benjamin lo instale A MANO** (parar servicio → MSI → arrancar servicio).*
>
> ***LO PRIMERO: ¿instaló ya la 0.1.43?*** Si NO → es lo primero, y **NO se publica la release de GitHub hasta que la tenga puesta** (ver el aviso de abajo). Si SÍ → verificar jar en disco = 0.1.43 + `runtime\lib\modules` = 119,3 MB + servicio Running + health 200, y entonces sí publicar la release 0.1.43.*
>
> ***⚠️ POR QUÉ NO SE PUBLICA LA RELEASE TODAVÍA (descubierto el 16-jul, NO estaba en el plan):** la 0.1.42 instalada de Benjamin **comprueba updates sola al arrancar** (`BenjagestUiApplication.enterApp` → `checkForUpdates(false)`) y ofrece "¿Descargarla e instalarla ahora?". Ese diálogo usa el actualizador **VIEJO** — el que rompió la instalación 3 veces el 15-jul. **Publicar la release ARMA la bomba en su app**: no basta con "acordarse de instalar a mano", porque su propia app se lo va a ofrecer sola. Orden obligatorio: (1) instala la 0.1.43 a mano desde el fichero → (2) ya tiene UPD-3 dentro → (3) ENTONCES se publica la release → (4) el actualizador NUEVO se prueba de verdad en 0.1.43 → 0.1.44.*
>
> ***LUEGO: probar el ciclo real del actualizador nuevo** (0.1.43 → 0.1.44), que es lo único de UPD-3 que sigue sin verificarse. Mirar `%ProgramData%\BENJAGEST\logs\update.log` (UPD-3 ya deja log desde la 1ª línea): a quién espera, qué devolvió msiexec, si el modules quedó bien, si queda algo en la cola. Si sale 3010 = ficheros en uso = sigue habiendo un tenedor.*
>
> ***PENDIENTE del 130 (otro slice)**: saldar la 473 contra la cuenta del titular (550) en el cierre — el IRPF de un autónomo es personal, no gasto de la actividad. La 473 ya acumula bien (verificado).*
>
> ***DEUDA anotada**: `TaxLedgerService` (303 + 130) **no tiene NI UN test automático** — el commit `92d60e1b` lleva prefijo `test(tax)` pero solo documenta verificación e2e, no añade fichero. Todo el puente fiscal→contable depende de que alguien repita el e2e a mano. Montar el harness (Spring + BD) es un slice en sí mismo.*
>
> ***Entorno e2e** (probado otra vez el 16-jul, funciona): Docker `benjagest-mariadb` (3307) → BD `benjagest_v8test` → backend 8081 con `BENJAGEST_DB_URL`+`BENJAGEST_BACKEND_PORT`. Usuario `e2e-asi4@example.test` / `PruebaE2E2026$`. **OJO con los nombres de tabla**: son `journal_entries` / `journal_entry_lines` (NO `accounting_journal_*`), y la FK de la línea es `journal_entry_id` (NO `entry_id`). El módulo `tax` y `accounting` ya quedaron activos en esa BD."*
>
> **Sesión 2026-07-16 (mañana) — UPD-3b + merge de UPD-3 + el 130 + MSI 0.1.43.**
>
