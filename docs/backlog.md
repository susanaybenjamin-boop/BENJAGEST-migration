...
> **Última actualización: 2026-07-13 — ✅ v0.1.38 PUBLICADA + AUTO-UPDATE VALIDADA + main=develop. La auto-actualización 0.1.37→0.1.38 funcionó SOLA por la vía de la app (launchInstaller para el servicio, instala, arranca; sin parar backend a mano, sin reinicio, lib\modules intacto) → el fix del instalador queda validado end-to-end y el incidente 0.1.36 cerrado. v0.1.38 lleva: concepto extraído no se sobrescribe + botón "Copiar texto del PDF" (PDFBox, capa de texto) + BARRIDO i18n de claves crudas en columnas (worklog SUBMITTED, ss_regime ARTISTAS/MAR/AGRARIO, customer_type MANAGED_CLIENT/INTERNAL, 9 module.*). **RAMAS: main reseteada a develop (676408c7) con force-push autorizado → main = estable/producción (0.1.38), develop = integración/default.** Pendiente Benjamin: smoke visual del concepto + botón copiar-texto. Histórico abajo.**
>
