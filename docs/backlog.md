...
>
> *"Lee la cabecera de docs/backlog.md y docs/plan-estructural-fases.md. Estamos en FASE 1 del plan estructural. Última release **v0.1.34** (tag develop `e9ee3486`), Benjamin confirmó 'todo ok' en producción. El **BLOQUE BANCARIO queda CERRADO** (Nueva cuenta + import de extracto AUTODETECTADO Excel/CSV/N43 en un botón + pantalla de REVISIÓN de conciliación con checkbox y el pago existente colgando debajo + botones Conciliar/Ignorar + errores legibles), smoke-eado end-to-end; en 2 sesiones (v0.1.33 y v0.1.34) el verificar-en-ejecución cazó 4 bugs (candidata ventas rota por total_amount/customer_legal_name; purchase_invoices sin payment_status; ck_bib_format sin XLSX; ignore con '||' en vez de CONCAT). Pendientes en orden: (1) **cuadre 130 2T con la asesoría** — BENJAGEST da pago 693,36 con la nómina de Rubén dentro; a la asesoría le sobran +580 de ingresos (¿abono no restado?) y ~1.477 de gastos (¿duplicado Los Llanos suyo, IVA no deducible, RETA?) — es reconciliación con la asesoría, NO código; (2) **F1-WINSVC + F1-INSTVAR** CON Benjamin delante — backend como servicio de Windows + 2 instaladores (Asesoría completo / Empleado solo-UI apuntando a LAN); necesita reinicios y un 2º PC; (3) **VF3-CERT** cuando quiera Benjamin — VeriFactu real con su certificado FNMT (localizado); sesión conjunta, necesita su contraseña; (4) **F1-XDIARIO** aparcado sin fichero de referencia. Gotchas de sesión: X-Company-Id ad9cb9e6-51bd-4556-8c54-a6f1ecc028d5 para la empresa de Benjamin (pin-login entra como la ASESORÍA); NO tocar 8080/13307 con su app abierta (el arranque embebido mata su MariaDB); smoke con arranque embebido -Dbenjagest.db.embedded=true -Duser.home=C:/Proyectos/smoke-home (FUERA de Temp) → tenant sintético (companies + user_accounts con bcrypt + company_memberships role_name OWNER + company_modules accounting activo + bank_accounts) y login POST /api/auth/login (el token viene en el campo **accessToken**, no 'token'); el seed demo V3 está ANULADO (BD limpia arranca vacía); release = bump UpdateService.APP_VERSION + build-msi.ps1 -Rebuild (WiX NO en PATH → prepend 'C:\\Program Files (x86)\\WiX Toolset v3.14\\bin') + gh release --target SHA_completo."*
>
