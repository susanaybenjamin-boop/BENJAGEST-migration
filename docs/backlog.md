...
>
> **Última revisión:** 2026-06-03 (VF2 verificabilidad cerrado + sección Base legal SIF/VeriFactu con RD 1007/2023 + Orden HAC/1177/2024 anotada + slices VF-EVENTS / VF-OFF-DEPRECATE / VF3-QR / VF-SIGN / VF-ANOMALY desglosados).
...
- 🔵 Hash encadenado + firma XML + reintentos (parcial):
  ...
  - ✅ **VF2** Verificabilidad de la cadena hash + huella en PDF (2026-06-03, `531cd4f`) — bugfix de truncado `generation_time` a segundos antes del hash y persistido explícito (sin ese fix, cualquier verify daría falso positivo de "cadena rota" por desfase nano vs `DEFAULT CURRENT_TIMESTAMP`). Endpoint `GET /api/billing/verifactu-registry/verify?mode=TEST` que recorre la cadena en orden cronológico, recalcula hash con (NIF, número, fecha, vatTotal, total, hash_previous esperado, `generated_at`) y devuelve la primera factura sospechosa si rompe. PDF muestra los 16 primeros caracteres del SHA-256 bajo el placeholder QR. UI: botón "Verificar integridad" en Configuración Facturación con mensaje OK (N comprobadas) / KO (primera sospechosa + razón) / OFF (pide activar modalidad antes). i18n ES+EN. **Pendiente VF2.1 (incumplimiento legal actual)**: el hash hoy solo se genera si `mode != OFF`, pero la ley exige cadena también en NO VeriFactu → cuando se cierre `VF-OFF-DEPRECATE` el hook quedará siempre activo.
  - ⬜ **VF-EVENTS** Registro de Eventos del SIF encadenado (obligatorio en NO VeriFactu — RD 1007/2023 + Orden HAC/1177/2024, ver bloque legal arriba). Port de la pestaña Auditoría de CONTENDO (JS→Java) para conservar paridad probada con AEAT (~25 facturas en TEST hasta que AEAT aceptó hash+QR). Migración nueva (`V18__sif_event_registry.sql`) con tabla `sif_event_registry` (id, company_id, event_type, payload JSON, hash_current, hash_previous, generated_at, signed_at, signature_data, status). Cadena independiente de la de facturas. 9 tipos obligatorios (ver bloque legal) + SUMMARY_6H + SUMMARY_SHUTDOWN. Servicio `SifEventService.record(eventType, payload)` que: (a) calcula hash encadenado, (b) firma (XAdES local mientras VF-SIGN no esté), (c) inserta. Hooks en arranque/parada Spring (`@PostConstruct` + `@PreDestroy`), `SalesInvoiceService.validate/void` (operación completada), exportaciones masivas, restore. Job `@Scheduled(fixedDelay=6h)` emite SUMMARY_6H. UI: nueva pestaña "Auditoría SIF" en Configuración con listado filtrable + botón Verificar integridad + botón Exportar período (genera evento tipo 9 al hacerlo). Tarea #54.
  - ⬜ **VF-OFF-DEPRECATE** Eliminar el modo `OFF` del combo de configuración VeriFactu — **no es modalidad legal**. Sustituir por `VeriFactu` / `NoVeriFactu` (con sub-selector TEST/PROD para VeriFactu). Migración (`V19__verifactu_modality.sql`): empresas en OFF pasan a `NoVeriFactu` por defecto (cadena empieza desde ahí, no se rehace lo pasado). Hash y QR pasan a estar siempre activos. Tarea #55. Bloquea cierre de VF2.1 y precede a VF-EVENTS.
  - ⬜ **VF3-QR** Pintar QR oficial AEAT en el PDF (URL al servicio de validación con CIF emisor + nº factura + fecha + importe codificados según Orden HAC/1177/2024). Dependencia: librería QR (ej. `com.google.zxing`). Sustituir el placeholder `qrPlaceholder()` actual. Aplica a ambas modalidades.
  - ⬜ **VF3-SOAP** Cliente SOAP a AEAT (Apache CXF, modos TEST y PROD). Solo modalidad VeriFactu (en NO VeriFactu los registros se conservan localmente y AEAT los pide si quiere).
  - ⬜ **VF-SIGN** Firma XAdES-EPES con certificado real (Apache Santuario + BouncyCastle). Aplica tanto a facturas como a eventos del SIF (la firma precede al cálculo del hash y forma parte del input).
  - ⬜ **VF-ANOMALY** Job de detección de anomalías (eventos 3 y 5 del SIF). Recorre las dos cadenas (facturas + eventos) periódicamente, si encuentra rotura emite eventos 4/6 + alerta UI. Equivalente al `verifyChain` automatizado + persistido.
