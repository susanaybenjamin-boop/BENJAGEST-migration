...
> **Última actualización: 2026-09-05 (noche) — 🔧 v0.1.64 — BLOQUE CONTA: la contabilidad no cuadraba, y eran CUATRO bugs distintos.** Todo salió de Benjamin mirando PRODUCCIÓN, no de los tests: *"la contabilidad no cuadra... teniendo un cliente con todas las facturas cobradas, me sale en sumas y saldos un saldo deudor de la última factura, y así todos los clientes"*. Diagnóstico hecho con consultas de solo lectura contra la MariaDB embebida de producción (`127.0.0.1:13307`, datos en `C:\ProgramData\BENJAGEST\mariadb-data`; la clave está en `.dbkey` del mismo directorio). **CONTA-1 — cobro/pago contra la cuenta GENÉRICA**: la factura carga la subcuenta del tercero (`4300005 "Clientes - 3R Consultores"`, vía `TerceroAccountResolverService`) pero los caminos que la saldan buscaban la cuenta *solo por prefijo* con `findAccountByPrefix(companyId, "430")`, que devuelve el código más corto = la genérica. Debe en una cuenta, haber en otra. Datos reales: `4300005` con +1.724,25 y la `430` con −1.724,25, el mismo importe con el signo cambiado. El fallo estaba **copiado en dos sitios** (`PaymentScheduleService` y `BankMovementService:130`), de ahí que la resolución se extraiga a una pieza compartida, `InvoiceCounterpartyAccountResolver`. La cuenta se saca del **asiento de la propia factura**, no volviendo a preguntar al resolver: si el asesor la reclasificó, el cobro la sigue. **CONTA-2 — el 130 a cero**: la 473 "H.P. retenciones y pagos a cuenta" recoge DOS cosas —las retenciones de clientes Y los pagos fraccionados del propio 130— y se sumaba entera como "retenciones" mientras los pagos previos se restaban aparte desde `tax_filings`: **se restaban dos veces**. En su BD la 473 tenía exactamente los dos pagos del 130 (827,04 + 693,36 = 1.520,40) y ninguna retención real. Ahora se excluyen los asientos `TAX_PAYMENT`/`TAX_LIQUIDATION` y el T3 pasa de 0 a **543,81**. **CONTA-3 — los informes no se refrescaban**: *"lo he reclasificado al banco, pero no se actualiza en sumas y saldos, en cambio en balance de situación sí"*. La acción SÍ emitía `TOPIC_JOURNAL`; lo que faltaba era que Sumas y saldos, Balance y PyG **escucharan**. **CONTA-4 — el más grave, y lo destapó él**: *"en libro mayor la 430 está a cero, pero si me voy a sumas y saldos, la genérica 430 tiene un saldo acreedor de 6112.92"*. Esos 6.112,92 eran **exactamente los cinco cobros ANULADOS** de esa cuenta. Causa: el filtro de estado vivía en el `ON` de un `LEFT JOIN` — si el asiento no es POSTED el join con `journal_entries` falla y `je` queda a NULL, **pero la línea sigue ahí** y `SUM(l.debit)` la cuenta igual; y al quedar `je.entry_date` a NULL esas líneas se colaban también por el filtro de fechas. El Libro Mayor no lo sufría porque hace un `JOIN` normal — de ahí la discrepancia entre pantallas que dio la pista. Estaba en TRES consultas (`JournalQueryService.balance`, `FinancialReportsService.balancesAt` y `balancesBetween`). **Alcance medido**: no era solo la 430 — contaminaba proveedores (1.660,58), bancos (6.112,92/4.316,49), **ventas** (`700` y `705`, 1.452 cada una) e IVA (`477` 609,84). Verificación final sobre producción ejecutando la consulta vieja y la nueva lado a lado contra el Mayor: **16 cuentas no cuadraban; ahora 0**. **Datos reparados** (con copia previa en `C:\ProgramData\BENJAGESTackup` y `DESHACER.sql`): 8 apuntes reclasificados de la genérica a su tercero, 6 de vencimientos y 2 de conciliación bancaria. **DIA-IMP y SYS-DESGLOSE** (petición suya): filtro por importe en el Diario (acepta "1.724,25", "1724.25" y solo la parte entera) y botón "Ver apuntes" en Sumas y saldos que abre el desglose de la cuenta reusando el endpoint del Mayor — pensado para cuando una genérica acumula importes de varios terceros. **Dos meteduras de pata propias**, ambas cazadas por él probando: el desglose lo puse solo en un doble clic sin afordancia (el Label de ayuda se creaba pero nunca se añadía al layout), y dos claves i18n no llegaron a añadirse, así que la cabecera salía como `accounting.col.number`. Se hizo después una **pasada completa de claves usadas vs definidas** en toda la UI (3.722 / 4.485), que además destapó `labor.editor.save`, sin definir desde siempre en `ContractTemplatesScreen`. **Pendiente, decisión de Benjamin (datos, no código)**: un pago de 51,20 a SH Asesores el 19-07 sin factura detrás, y el NIF `B19682756` dado de alta con dos nombres ("SH Asesores Vergeles" y "SH QR Tributario") con una cuenta cada uno, más 5 facturas de ese proveedor sin NIF. **Lección de método**: no recompilar con la UI abierta (`mvn compile` reescribe `ui/target/classes` bajo el proceso vivo y provoca `NoClassDefFoundError`, que parece un bug de la app). Suite final: 211 tests, 0 fallos.

> **Sesión anterior 2026-09-05 (tarde) — ✅ v0.1.63 PUBLICADA — 🔧 v0.1.63 — NUM-1 (la serie de facturación estaba ATASCADA), COB-5 y M130-3.** Todo salió de probar la 0.1.62 con la app delante, no de los tests. **NUM-1 — el bug gordo del día**: validando una factura, 500 con error SQL crudo (`Duplicate entry '...-FRA-2026-0008' for key 'uk_sales_invoices_company_number'`). Benjamin lo dio por explicado ("era histórica, de haberla importado ya existentes"), pero el código dice otra cosa: las históricas entran ya como `VALIDATED` y `markValidated` lleva `AND status = 'DRAFT'`, así que validar una histórica habría dado un 409 limpio, no un error SQL. La que fallaba era **una factura nueva**, y la histórica le había robado el número. Consulta de solo lectura contra su BD: serie FRA con `next_number = 8` y 9 históricas ocupando de `FRA-2026-0001` a `FRA-2026-0008`. **Y estaba atascada para siempre**: `claimNextNumber` es `@Transactional` y va dentro de la transacción de `validate`, así que sube el contador a 9, el UPDATE choca, la transacción hace rollback y **el contador vuelve a 8** — el reintento pide el mismo número. Con la serie en ese estado no se podía emitir NINGUNA factura nueva. Origen del desfase: el import de PDF y el de CONTENDO insertan las históricas por SQL directo con el número del papel y `series_id` NULL, sin tocar el contador — es a propósito (línea roja VeriFactu/SIF, no pueden pasar por `SalesInvoiceService`), pero nadie lo adelantaba después. **Arreglo**: `claimNextNumber` salta los números ya ocupados en la empresa antes de emitir, consultando contra toda `sales_invoices` (la restricción que revienta es `(company_id, invoice_number)` y las históricas ni tienen `series_id`). Saltar NO abre hueco: ese número existe, lo tiene la histórica; la serie completa sigue siendo correlativa, que es lo que exige la norma. Es **autorreparador** — la serie de Benjamin se desatascó sola en el siguiente intento, sin tocar la BD a mano. Cinturón a las 10.000 vueltas por si el formato de la serie no distingue números. **COB-5**: COB-2 solo llegó al listado del cliente NO vinculado; los VINCULADOS usan `ClientBillingScreen` (versión reducida) y ahí seguía sin poder cobrarse. Botón "Vencimientos / Cobro" añadido; aquí las filas ya son facturas, así que basta su id (sin el rodeo por `source_id`). Con esto el cobro está en los cuatro listados de factura de venta. **M130-2**: la tarjeta del 130 solo se había puesto en el cuadro del cliente NO vinculado; los vinculados y "Mi gestión" usan `AccountingScreen` → Cuadro de mando, que tiene su propia tarjeta del 303 vía `ClientFinancialsService`. Como ese servicio ya delega en `SalesAndExpensesKpiService`, solo hubo que dejar pasar los campos. **M130-3**: *"cuando filtro las fechas el 303 se actualiza de lujo, pero el 130 no, está fijo en todo el año"*. Dos fallos de M130-1: la tarjeta solo miraba el `hasta` y siempre sumaba desde enero (y el cuadro arranca con desde = 1 de enero, así que parecía congelada), y los pagos previos salían de `tax_filings` solo si estaban presentados, contando 0 e inflando la cifra. Ahora muestra el pago **del trimestre** del `hasta`: acumulado de enero menos los trimestres anteriores, calculados en cascada — manda el importe PRESENTADO en Fiscal si existe, y si no se calcula de la contabilidad. Se añade `model130Quarter` y las tarjetas lo pintan ("estimado, T2"), que era medio problema: no había forma de saber a qué periodo iba la cifra. **NO se calcula con las fechas exactas del filtro** aunque sería lo "consistente" con el 303: el 5% de difícil justificación tiene tope ANUAL y los pagos previos solo tienen sentido sobre el acumulado, así que un "de abril a junio" aislado no se correspondería con ninguna casilla real. **Fix de UI encontrado por el smoke test**: al añadir el campo Concepto, el texto de ayuda con `wrapText` reclamaba todo el ancho y aplastaba la columna de etiquetas hasta dejarla en "…" — "no se leen las cabeceras de las líneas en el cuadro de pago". Arreglado con `ColumnConstraints` explícitas en los dos diálogos. Compilaba y los 192 tests seguían verdes: solo se ve abriéndolo. **Lección de método**: no recompilar con la UI abierta — `mvn compile` reescribe `ui/target/classes` bajo el proceso vivo y provoca `NoClassDefFoundError` en clases aún no cargadas (pasó con `TeamScreen` y parecía un bug de la app). Suite final: 200 tests, 0 fallos.

> **Sesión anterior 2026-09-05 (mañana) — ✅ v0.1.62 PUBLICADA — 🔧 v0.1.62 — BLOQUE COB (cobrar/pagar desde asesoría, con concepto propio) + M130-1.** Benjamin: *"este modo puede introducir facturas por pdf con el contrato firmado previo, o por asiento, y validarlas, pero no tiene si abre una venta validada, la opción de cobrarla, y en el caso de las compras igual"*. **Primer hallazgo: la premisa de compras era falsa.** La pestaña Gastos del cliente es `buildPurchasesListing(false)` — literalmente la MISMA pantalla que el modo empresario, con "Registrar pago" y "Vencimientos" ya puestos. Si a Benjamin no le salían, era el gate del acuerdo TPB (`ensureBillingAllowed`, `BenjagestUiApplication:7466`): sin contrato firmado para el ámbito de compras esos botones se apagan. Ahí no había nada que portar. Lo que faltaba de verdad era **solo ventas archivadas**. **COB-1**: la UI ya recibía `source_id` del backend (`JournalQueryService.diary` lo selecciona desde siempre) pero `AccountingModels.DiaryEntry` no lo parseaba — ese campo es el que enlaza el asiento con su factura. **COB-2**: botón "Vencimientos / Cobro" en Ventas (archivadas), reutilizando el MISMO diálogo del modo empresario (`openDueDatesDialog`), sin duplicar nada: la factura importada por PDF ya crea su fila HISTORICAL en `sales_invoices` y deja el asiento con `source_type=SALES_INVOICE` + `source_id` (`SalesPdfImportService:216`). Solo se habilita si HAY factura enlazada — una venta metida como asiento manual no tiene fila en `sales_invoices` y por tanto no puede tener cuadro de vencimientos (decisión de Benjamin entre tres opciones); en ese caso el tooltip explica por qué está apagado en vez de dejarlo mudo. **COB-3 y COB-4**: el concepto del asiento de cobro/pago pasa a ser editable, en los dos modos y en los DOS caminos de pago que tenía la app — el cuadro de vencimientos (`PaymentScheduleService:369`, fijaba "Cobro vto. N" / "Pago vto. N") y el botón "Registrar pago" de Compras (`PurchaseJournalEntryService:333`, fijaba "Pago Fra. N - Proveedor"). Los dos campos vienen **prerrellenados con ese mismo texto**, así que quien no los toque obtiene el asiento idéntico al de antes, y `concept` es opcional en ambos `PayRequest` (un cliente que no lo mande sigue funcionando). **M130-1**: tarjeta "MODELO 130 (est.)" junto a la del 303 en el cuadro de mando del cliente. Calculada desde los ASIENTOS igual que las otras cinco tarjetas (decisión de Benjamin), para que las cifras del cuadro cuadren entre sí — el 130 oficial de Fiscal suma desde `sales_invoices`/`purchase_invoices` y puede diferir si hay ventas metidas a mano. La fórmula NO se duplica: se reusa `AeatExtraModelsService.compute130`, que ya era pública y estática, para no tener el 5% de difícil justificación, el tope de 2.000 € y el tipo del 20% en dos sitios. Es **acumulado del año** hasta el fin del rango, no del trimestre suelto (el 130 se declara así). Solo se pinta para quien lo presenta — autónomo en estimación directa (`companies.legal_form` V120 / `client_advisory_config.tax_regime` V119); una SL o alguien en módulos no la ve, porque un "IRPF a pagar" en la ficha de una sociedad sería información falsa. Con la ficha sin rellenar se muestra igualmente (fail-open). **Pifia propia evitada a tiempo**: asumí que la tabla era `client_config` y en realidad es `client_advisory_config` — lo cazó leer la V119 antes de commitear, exactamente lo que manda §10.bis. **Verificación en ejecución (§10.ter)**: test de integración nuevo `CobroConceptoYModelo130IntegrationTest` sobre MariaDB embebida con las 181 migraciones — 12 casos verdes (cobro con concepto propio, cobro sin tocarlo → "Cobro vto. 2" clavado, el 130 dando 940,00 con la fórmula legal, acumulado del año y no del trimestre, SL/módulos sin tarjeta, ficha vacía con tarjeta, y el contrato JSON en los dos `PayRequest` + la serialización de los campos nuevos del KPI). Suite completa 192 tests, 0 fallos. **Lo que NO se pudo verificar**: abrir las pantallas JavaFX — las contraseñas demo de la V8 ya no valen en esa BD (401) y no se tocaron hashes de Benjamin para forzar un login.

> **Sesión anterior 2026-09-02 — 🔧 v0.1.61 — BLOQUE REC-CRON-AUTH (los gastos recurrentes NUNCA se ejecutaron solos) + TRB-G5.** Benjamin: *"no sé por qué, no se están ejecutando los gastos recurrentes y las ventas recurrentes, ¿puedes auditarlo?"*. **REC-CRON-AUTH — la causa raíz, con evidencia en producción**: el cron corre en el hilo `scheduling-1`, donde el `SecurityContext` está VACÍO (no hay petición HTTP ni JWT), y `PurchaseInvoiceService.save()` llama a `currentUserService.require()` **sin proteger** → `401 UNAUTHORIZED "No hay sesion activa"`. La prueba está en `recurring_task_runs` de la BD real: **solo hay OK en las ejecuciones manuales** (run-now, con JWT: 07-07, 08-08, 08-13) **y ERROR en todas las del cron** (08-04 y 09-01). Es decir: la ejecución AUTOMÁTICA de recurrentes no funcionó ni una sola vez. El resto del motor sí estaba preparado para el cron (`BillingAgreementGuard` hace `catch → return` con el comentario *"sin contexto de usuario (cron, scheduled, hooks)"*, y `ManualJournalEntryService`/`SalesInvoiceService` envuelven su `require()` en try/catch); **esa llamada se quedó fuera**, y existe desde `6866e6bd` (2026-06-05), antes de que hubiera motor de recurrentes. **Sobre las ventas recurrentes**: Benjamin las daba por rotas por analogía, pero en la BD **no había ni una sola tarea `SALES_INVOICE`** — no es que no se ejecutaran, es que no existían. Su camino (`SalesInvoiceService.createDraft`) ya era cron-safe. **El arreglo (opción A de las dos que se le plantearon)**: clase nueva `RecurringRunIdentity`, que monta el `SecurityContext` del usuario que creó la tarea (`recurring_tasks.created_by_user_id`) antes de ejecutarla. Se eligió frente a capar el `require()` porque así la ejecución automática es **idéntica a la manual** (mismo `uploaded_by_company_id`/`uploaded_by_user_id` en la factura, misma auditoría, mismos guards) y **cubre de una vez cualquier otro `require()` sin proteger** de las rutas de compras, ventas y asientos (hay ~70 en el backend). La empresa activa se resuelve igual que en el login: la membership en la empresa de la tarea si existe, y si no la primera — que es el caso de la asesoría que creó la recurrente *actuando como cliente*. Solo lee de `AuthRepository`: **no toca `AuthService` ni los claims del JWT** (CLAUDE.md 11.2). El handle **restaura** la autenticación anterior en vez de limpiarla, porque `runForDate` también lo invoca el endpoint `run-all` sobre el hilo de una petición HTTP. **Segundo defecto, igual de caro**: `finalize()` avanzaba `next_run_date` **también en ERROR**, así que el periodo fallido se perdía en silencio para siempre — la cuota de autónomo que falló el 08-01 saltó directa al 09-01 y ese gasto **nunca se generó**. Ahora un ERROR deja la fecha donde está y se reintenta a diario (OK y SKIPPED sí avanzan). **Tercero**: un fallo era invisible. Ahora emite aviso en la bandeja (la campana del header) con el motivo real, **solo en la transición a fallo** — si no, con el reintento diario sería un aviso cada día. Asesoría → `advisory_notifications`, empresa → `business_notifications`, mismo criterio de `company_type` que usa la campana. **VERIFICADO EN EJECUCIÓN** (regla 10.ter): backend en 8099 contra la MariaDB de desarrollo del 3307 y cron cada minuto, **sin tocar el servicio de producción** (siguió RUNNING todo el rato). El cron (hilo `scheduling-1`) sacó *"3 tareas para 2026-09-02 → 2 OK, 1 ERROR"*, **ni un 401**; el gasto recurrente generó su factura DRAFT de 328,11 € con los comodines `{MES}{YYYY}` expandidos y `uploaded_by_user_id` = el creador; la tarea rota a propósito dio ERROR con su mensaje de negocio, **dejó `next_run_date` sin avanzar, se reintentó 3 veces y emitió UN SOLO aviso**. Datos de prueba borrados al terminar. Commit `5d762620`, merge `9fc858e5`. **TRB-G5 — el bug de Trabajos que Benjamin cazó de paso**: duplicó un trabajo de 3R CONSULTORIA y al desplegar el cliente no aparecía; lo atribuyó al cambio de horas. **No eran las horas**: el duplicado quedó fechado el **2026-09-01** y los otros 17 son de agosto. El listado tiene DOS consultas que no coincidían — las filas hijas salen de la consulta **CON** rango de fechas y el aviso de la cabecera de TRB-G3 sale de la segunda consulta **SIN** rango. Por eso la cabecera avisaba del pendiente y el detalle no lo enseñaba por ningún lado. **Decisión de Benjamin**: *"las fechas siempre en blanco hasta que el usuario las cambie, y si se sale de trabajos y vuelve a entrar, otra vez en blanco, así no habrá conflicto de fecha"*. Las dos fechas arrancan vacías y **en blanco = SIN filtro** (se ve todo); cada fecha es un extremo abierto por separado; como `buildWorkLogsModule()` se reconstruye al entrar al módulo, salir y volver las deja en blanco solo. Y si SÍ se pone rango, los pendientes de fuera cuelgan igualmente de la cabecera, al final, **en cursiva y atenuados** con *"fuera de las fechas filtradas"*; van en `outsideJobs`, **aparte de `jobs`**: no suman en los agregados ni entran en la selección masiva de la cabecera (el filtro sigue mandando en lo que se factura de golpe), pero se pueden seleccionar una a una. Un cliente que SOLO tenga trabajos fuera del rango también tiene cabecera. 3 claves i18n ES+EN + `.worklog-outside-row` en `app.css`. **VERIFICADO EN EJECUCIÓN** pilotando la UI real con capturas, sobre el caso real de Benjamin y solo listando: al entrar, *"Cualquier fecha"* en los dos selectores y 3R CONSULTORIA con **18 trabajo(s)** — al desplegar, el primero es el 2026-09-01 de 9,00 h × 25,00 € (justo el que desaparecía); filtrando 01-08 a 31-08, cabecera *"17 trabajo(s) (+1 fuera del rango)"* y el 2026-09-01 el último de la lista, atenuado; saliendo a Inicio y volviendo, fechas en blanco de nuevo. Commit `87017b07`, merge `09617b02`. **PENDIENTE (decisión de Benjamin)**: los 4 gastos recurrentes perdidos (cuota RETA 328,11 € y SH ASESORES 51,20 €, del 01/08 y el 01/09) **los mete él a mano** — se le ofreció recuperarlos moviendo `next_run_date` y dijo que no. Las dos tareas siguen en `next_run_date = 2026-10-01`, así que **la primera ejecución automática real será el 1 de octubre**. **NOTA de despliegue**: el backend nuevo se instaló en el servicio de Windows en caliente (jar viejo guardado como `backend.jar.bak`); arrancó limpio (*Started BenjagestBackendApplication in 13.399 seconds*, Tomcat en 8080) y `RecurringRunIdentity.class` está dentro del jar desplegado. 180 tests, 0 fallos. **PENDIENTE MENOR**: en Trabajos, cambiar una fecha sigue exigiendo pulsar "Refrescar" (comportamiento que ya existía, no se tocó). **MISMO PATRÓN SIN REVISAR**: `BoeAlertService` y `ContractAlertService` también son `@Scheduled` a las 06:00 — no se auditó si alguna de sus rutas llama a `require()` sin proteger.**
>
> **Sesión anterior 2026-08-24 — ✅ v0.1.59 y v0.1.60 PUBLICADAS — BLOQUE TRB-G: TRABAJOS AGRUPADOS POR CLIENTE, PLEGABLES.** Idea de Benjamin: *"que los trabajos se filtren por clientes, y se plieguen y se desplieguen al cliquear encima del cliente, y cuando estén plegados se vea información de interés"*, pidiendo opinión honesta de si merecía la pena. **El veredicto fue que el plegado por sí solo es cosmético**; lo que lo justifica son las dos cosas que habilita: una vista panorámica de cuánto se le debe facturar a cada cliente (antes había que ir cliente por cliente con el combo) y arreglar el flujo de FACTURAR, que exigía ctrl+clic fila a fila cuidando de no mezclar clientes (`sameCustomerBillable` ya obligaba a que todo fuese del mismo cliente). Decisiones de Benjamin: agregado = **pendiente Y total**, y **siempre por cliente** (sin selector de agrupación, menos piezas que romper). **TRB-G1**: el `TableView` plano pasa a `TreeTableView` (mismo patrón que el árbol de conciliación de `ClientFinancialsScreen`, un nodo envoltorio con dos caras: cabecera de cliente o trabajo). La cabecera lleva última fecha · nº de trabajos · desglose de estados · **Pendiente** · **Total** · "Por facturar"/"Al día"; clic sobre la fila del cliente pliega/despliega; se **recuerda qué grupos tenías abiertos** entre refrescos (si no, cada acción te cerraba el árbol en la cara); botones Desplegar/Plegar todo; orden por más pendiente primero y "Sin cliente" al final. **TRB-G2**: seleccionar la CABECERA selecciona todos los pendientes de ese cliente → Facturar de un clic. **TRB-G3**: segunda consulta SIN rango de fechas para avisar de lo pendiente que queda FUERA del filtro (en la cabecera del cliente y en el pie) + botón "Todo lo pendiente" que estira las fechas hasta cubrirlo todo; si esa consulta falla NO rompe el listado. Cierra la trampa de leer "0 pendiente" en un cliente que sí lo tiene, pero de otro mes. **Fix de paso**: la columna Empleado salía VACÍA en los trabajos del titular — el parser devuelve `""` (no `null`) y el código sólo comprobaba `== null`; ahora pone "Yo / titular" (la misma trampa que ya mordió en `sameCustomerBillable`). 13 claves i18n ES+EN; en `app.css`, las filas de `TreeTableView` **no** llevan `.table-row-cell`, así que hubo que repetir los estilos de `.data-table` para `.tree-table-row-cell` + `.worklog-group-row`. **Sin cambios en backend ni BD.** **VERIFICADO EN EJECUCIÓN** en entorno de PRUEBA (backend en 8081 contra la MariaDB de Docker 3307, sin tocar el servicio de producción del 8080), pilotando la UI real con capturas: agrupación y agregados correctos; clic en el cliente pliega/despliega; Facturar con la cabecera seleccionada creó la factura borrador de GONZALO (1.250 €) y la vista se refrescó SOLA a "Al día"/"Facturado" sin pulsar Refrescar, conservando el grupo desplegado; filtrando septiembre salió "Ojo: hay 3.750,00 € pendiente de facturar fuera de las fechas filtradas"; "Todo lo pendiente" estiró el rango a 05-08/07-08 y sacó los 3 pendientes; tras reiniciar, Empleado muestra "Yo / titular". Commit `fe6ca54d`, merge `e7e2ea73`. PENDIENTE MENOR: el endpoint `/api/work-logs` tiene `LIMIT 500` (`WorkLogService.listForCompany`), así que la consulta de "todo lo pendiente" se cortaría a partir de 500 trabajos pendientes — hoy no afecta. NOTA de entorno: el `~/.gitconfig` global de Benjamin no existe (git no sabía quién era al commitear); se fijó identidad **local** en este repo. **TRB-G4 — v0.1.60, tres cosas que Benjamin caza USANDO la 0.1.59** (commit `58639e2d`): (1) *"para mostrar todos los trabajos tenemos que saber las fechas del primer trabajo"* — "Todo lo pendiente" marca la casilla de pendientes y estrecha el rango, y al desmarcarla te quedabas con el rango corto. Botón hermano **"Todos los trabajos"**: estira el rango al primer y último trabajo que existan, DESMARCA "Solo pendientes de facturar" y limpia el filtro de estado; consulta el rango completo al pulsarlo, no en cada refresco. (2) Con un único cliente el grupo salía DESPLEGADO — fuera las dos reglas de auto-despliegue (grupo único y filtro por cliente); ahora solo queda abierto lo que abre el usuario, que es como le gusta. (3) **Fuera los LIMIT** de las consultas de trabajos: `LIMIT 500` en `listForCompany` (con el tope, los botones "Todo lo pendiente"/"Todos los trabajos" mentirían pasados 500 trabajos) y `LIMIT 200` en `listMine` (PWA del empleado) — decisión de Benjamin, sin límite. VERIFICADO EN EJECUCIÓN igual que el resto (UI real, capturas): "Todo lo pendiente" → 05-08/07-08, casilla marcada, 3 trabajos y el único cliente PLEGADO; "Todos los trabajos" → casilla desmarcada, los 4 trabajos y los 2 clientes, ambos plegados. NO verificado en ejecución: `listMine` (PWA del empleado), solo compila.**
>
> **Sesión anterior 2026-08-16 — ✅ v0.1.58 PUBLICADA — CONC-3: dos bugs del concepto de la línea de factura, encontrados USÁNDOLO.** Benjamin, con una factura real: copió de un PDF un concepto de TRES líneas y al pegarlo *"en cada salto de línea no separaba la última palabra con la primera de la siguiente"*; no lo corrigió porque *"la columna concepto no se expande completamente para que lo leas todo bien, tienes que mover el cursor adelante y atrás"*. Lo descubrió cuando ya había enviado el PDF al cliente. **(1) El pegado**: la causa es de JavaFX — un control de UNA línea **descarta** los saltos al pegar y no pone nada en su lugar (`…mantenimiento` + `y revisión…` → `…mantenimientoy revisión…`). Se sobreescribe `paste()` y los saltos pasan a UN espacio ANTES de insertar (`InvoiceEditorScreen.singleLine`), con dos matices: un guion al final de línea es palabra partida por el ancho de página (`manteni-\nmiento`) → ahí NO se mete espacio, y **no se le quita el guion** (acertaría con las partidas pero rompería `pre-\nventa`, y no hay forma de distinguirlas); y los espacios repetidos del copiado se colapsan. Se aplica también al insertar línea desde un CONCEPTO guardado o desde un TRABAJO, y al campo Nombre del formulario de concepto. **(2) La lectura**: la descripción pasa de `TextField` a `TextArea` con `wrapText` (se lee entera en varias líneas), sigue siendo dato de UNA línea (Enter no la parte, el pegado normaliza) porque es lo que espera el PDF, + tooltip con el texto completo + columna 280→360. **VERIFICADO EN EJECUCIÓN**: 9 casos de `singleLine` contra la clase compilada (el caso exacto de Benjamin, saltos Windows, espacios alrededor del salto, palabra partida, espacios repetidos, línea en blanco, texto sin saltos, null y vacío) + UI real (Ctrl+V del texto de 3 líneas → queda bien separado; con texto largo la celda lo envuelve en DOS líneas y se lee entero) + borrador guardado y comprobado en BD: 157 caracteres y `LOCATE('\n')=0`, una sola línea limpia, que es lo que va al PDF. LECCIÓN: los dos bugs eran de USO real, no de código — ninguna prueba los habría encontrado sin pegar de verdad un texto de un PDF. Commit `9f618cad`.**
>
> **Sesión anterior 2026-08-14 (2ª) — ✅ v0.1.57 PUBLICADA — BLOQUE PAGO: VER QUÉ FACTURAS ESTÁN PENDIENTES DE PAGO.** Benjamin, tras actualizar a la 0.1.56 y probarla OK: *"en el cuadro de mando te dice pendiente de cobro y pendiente de pago; el cobro es fácil saber cuál es la factura, pero el pago no"*, y pidió resolverlo a criterio de Claude, con pruebas y release. **CAUSA DE DISEÑO**: "Pendiente de pago" (Contabilidad → Cuadro de mando) es el **saldo acreedor de las cuentas 400/410 del diario**, no una lista — la cifra es correcta pero no dice qué facturas la componen (`ClientFinancialsService.pendingPayments`; se hizo así tras la V45, que dropeó `payment_status` de `purchase_invoices`). **HALLAZGO GORDO al mirar el detalle — PAGO-1 `885fb86d`**: había **DOS verdades desincronizadas** sobre si un gasto está pagado, `purchase_invoices.paid` (botón "Registrar pago", GAS-2/V167) e `invoice_due_dates.status` (diálogo "Vencimientos / Pago", PV-1/V133), y **nadie las sincronizaba** (ventas sí lo tenía: `syncSalesPaymentStatus`). Dos consecuencias reales, la 2ª reproducida: (a) un gasto pagado por vencimientos figuraba PENDIENTE para siempre — cualquier listado nuevo habría mentido; (b) **se podía pagar DOS VECES** (pagar por vencimientos no marcaba el flag → "Registrar pago" lo aceptaba y generaba un SEGUNDO asiento 400→572; familia BANK-DUP). Fix: `syncPurchasePaidFlag` cableado en `pay()` + `settleByBankMovement()` + `unpay()`; el camino inverso (`PurchaseInvoiceService.settleDueDates`) salda los vencimientos pendientes enlazándolos al asiento ya creado; **V181** repara el histórico con criterio conservador (solo marca pagado el que TIENE vencimientos y los tiene TODOS PAID; jamás desmarca ni pisa fechas). **PAGO-2 `db66d956`**: las dos tarjetas del cuadro de mando son **clicables** (Pendiente de pago → Compras y Gastos filtrado por "Pendientes de pago"; Pendiente de cobro → Facturación con Cobro=Pendiente), vía `Router.navigateTo(module, intent)` (default method) + campo `pending*` que la pantalla consume, el patrón de `pendingBillingTab`. En Compras: **filtro "Pago:"** (todos/Pendientes/Pagadas) y **columna "Pagado"** — el backend ya devolvía `paid`/`paidDate` desde V167 y **la UI los tiraba** (`PurchaseInvoiceEntry` no los transportaba). El subtítulo dice cuántas son ("3 sin pagar · pulsa aquí"). 8 claves i18n ES+EN, sin CSS nuevo. **VERIFICADO EN EJECUCIÓN**: 4 tests de integración nuevos (migra hasta V180, siembra el estado ROTO, aplica V181 y comprueba reparación + los casos que NO debe tocar) + suite backend **180 verde**; sandbox real (la V181 arregló al arrancar la compra SP-2026-01 que estaba así de verdad) y por API: pagar 1 de 2 vencimientos NO marca la factura, pagar el 2º sí, "Registrar pago" sobre pagado → **409** (antes: doble asiento), deshacer → vuelve a pendiente; y **smoke de la UI JavaFX en MODO ASESORÍA** (cuenta de asesoría dada de alta en el sandbox, con capturas): Mi gestión → Contabilidad → Cuadro de mando → clic → las 3 facturas que suman exactamente los 949,85 € del KPI. Fallo cazado por el smoke: subtítulo demasiado largo para la tarjeta (210 px) → se cortaba con "…"; ahora van cortos y el dato manda sobre el "pulsa". OJO para quien siga: en modo asesoría **Contabilidad NO está en el sidebar** (`KNOWN_VIEWS` no la incluye), se llega por **Mi gestión → pestaña Contabilidad**.**
>
> **Sesión anterior 2026-08-14 (1ª) — ✅ v0.1.56 PUBLICADA — BLOQUE CONC: CONCEPTOS REUTILIZABLES EN LA FACTURA.** Petición de Benjamin: *"poder elegir conceptos que ya hayamos utilizado en anteriores facturas o hayamos creado"*, delegando en Claude implantar + probar + versionar. **HALLAZGO: NO hizo falta tabla nueva** — `catalog_items` existía desde la V2 con la FK `sales_invoice_lines.catalog_item_id` puesta y CERO clases Java usándola (el javadoc de `InvoiceLine` decía literal *"queda preparado para el slice futuro de catálogo de productos/servicios"*). Este bloque la enciende. **CONC-1 backend `d6e16c6d`**: V180 = ALTER aditivo (`default_retention_percent` + `default_vat_rate_id`, lo único que faltaba tras FAC-IVA/V175); `GET /api/billing/catalog-items` devuelve las DOS fuentes mezcladas — `source=CATALOG` (guardados) y `source=HISTORY` (agregados AL VUELO desde `sales_invoice_lines`, sin copiarlos a la tabla: así no hay dos sitios que se desincronicen); `usageCount`/`lastUsedAt` salen SIEMPRE del histórico real, también para los guardados; POST **idempotente por nombre**, PUT, y DELETE = **baja LÓGICA** (las líneas de facturas emitidas apuntan por FK y una factura validada no se toca); fuera las CANCELLED y, por TenantContext, las de otras empresas; el agregado se hace en Java sobre las líneas ordenadas de reciente a antigua (primera aparición = valores buenos) para NO usar funciones de ventana (este proyecto no usa ninguna y no estaban verificadas contra MariaDB4j). **CONC-2 UI `3e24e32c`**: en "Conceptos / Líneas" dos botones — **"Conceptos"** (selector con buscador sin tildes, columnas precio/IVA/ret./usos/último uso/origen "Ya facturado"·"Guardado", selección múltiple, "Añadir a la factura" vuelca una línea por concepto, y desde el mismo diálogo Nuevo/Guardar en catálogo/Editar/Quitar habilitados según CATALOG o HISTORY) y **"Guardar concepto"** (el camino inverso: la línea seleccionada de la factura se guarda como concepto). Si el concepto no trae IVA/retención propios se usan los del CLIENTE. 36 claves i18n ES+EN. Sin CSS nuevo. **NO se toca el guardado de la factura** (`upsertInvoice`/`InvoiceLineDraft` intactos): el camino crítico factura→validar→huella SIF no se roza. **VERIFICADO EN EJECUCIÓN, TODO** (Benjamin delegó las pruebas): 4 tests de integración nuevos sobre MariaDB embebida + suite backend **176 verde**; sandbox real (`benjagest_v8test`/8081) con V180 aplicada por Flyway y los 6 caminos por curl (GET agrupado, POST x2 sin duplicar, PUT, DELETE con `active=0`, 400 sin nombre, 400 IVA 150%, 404); y **smoke de la UI JavaFX de verdad, con capturas** (login→Facturación→Nueva factura→Conceptos→crear→añadir→Guardar borrador→línea comprobada en BD: 90 + 21% − 15% = 95,40 €). Ese smoke cazó **TRES fallos de layout** que "compilaba" no habría visto: (1) `prefWrapLength(MAX_VALUE)` hacía que la HBox de la cabecera encogiera la fila a su mínimo → los 5 botones **apilados en vertical** y el título truncado; (2) etiquetas del formulario cortadas ("No…", "Tex…") → `ColumnConstraints`; (3) textos de ayuda aplastados a una línea con "…" → `minHeight USE_PREF_SIZE`. PENDIENTE OPCIONAL: mandar `catalog_item_id` en la línea al guardar (se dejó fuera a propósito para no tocar el camino crítico). NOTA sandbox: el combo de IVA de las líneas sale en blanco en `benjagest_v8test` porque esa empresa no tiene `vat_rates` configurados — comportamiento previo, no del bloque.**
>
> **Sesión anterior 2026-08-07 — ✅ v0.1.55 PUBLICADA (Latest, target `7066ffd9`) — solo bump: Benjamin YA se había actualizado a una 0.1.54 intermedia y el actualizador no ve re-publicaciones del mismo número (lección: si Benjamin pudo actualizar, INCREMENTAR versión, no re-publicar). La v0.1.54 quedó publicada como histórico (mismo contenido). CONTIENE: (1) VF-QR-TOGGLE + fix huella `27e8e58f` (con el toggle apagado la factura sale limpia TOTAL: ni QR, ni etiqueta, ni placeholder, ni huella impresa; verificado BUENA-2026-0006/0007). (2) PDF-LOGO `74071fd7` — el logo de empresa (PORT-4 jun-2026) NUNCA se pintaba en el PDF (celda placeholder); ahora InvoicePdfGenerator inyecta CompanyLogoService y lo pinta escalado 140x50pt; verificado BUENA-2026-0010 (QR off: 1 imagen=logo) y 0011 (QR on: 2 imágenes). Lo detectó Benjamin en su 1ª factura real tras el toggle. (3) PDF-RESTORE `123de421` — si el PDF VALIDATED falta en su ruta (movido/borrado), GET /pdf antes regeneraba SOLO en memoria ("lo abre de la nada"); ahora storePdfNow lo regenera Y REESCRIBE en la ruta (truco: mover PDFs viejos y reabrirlos = se regeneran con logo); verificado borrando el archivo de 0011 → reaparece. Suite backend 172 OK tras cada cambio. OJO limpieza: el sandbox escribió PDFs de prueba en C:\ProgramData\BENJAGEST\facturas (root por defecto) — borrados; para futuros e2e fijar invoiceStorageRoot del sandbox a Temp. — VF-QR-TOGGLE `0f43bd36`: casilla en Facturación→Configuración para emitir facturas SIN el QR AEAT mientras la empresa no esté obligada (RD 1007/2023; fecha límite POR NIF: DNI/NIE 1-jul-2027, sociedades 1-ene-2027; sin NIF la más temprana). El backend caduca el toggle (lo ignora desde la fecha), la UI lo muestra bloqueado. Con QR apagado: ni QR, ni etiqueta, ni placeholder (zona vacía como proforma); la HUELLA se conserva. V179 = companies.verifactu_print_qr default TRUE. Cableado en validar/almacenar + previsualizar + email. CONTEXTO de la decisión: Benjamin NO va a terminar Benjagest como producto (en el año nuevo arranca una app de pago); la deja para SU contabilidad, y el toggle existe por si algún día la vende. VERIFICADO EN EJECUCIÓN (sandbox v8test/8081): V179 aplicada por Flyway, GET/PUT config con printQr+qrOptOutUntil=2027-07-01, factura BUENA-2026-0004 con QR (PDF 1 imagen) vs BUENA-2026-0005 sin QR (PDF 0 imágenes, sin placeholder, con huella — streams descomprimidos inspeccionados), VerifactuDatesTest + suite backend 172 OK. NO VERIFICADO: el click de la casilla en la UI JavaFX (solo puede Benjamin; el guardado va por el mismo PUT ya probado). Benjamin delegó las pruebas y autorizó publicar. Sesión anterior mismo día: v0.1.53 (módulo Trabajos).**
>
> **Sesión 2026-08-06 (CIERRE) — ✅ v0.1.53 PUBLICADA (Latest, MSI 302 MB, target = SHA develop `56dde95c`), smoke UI VERIFICADO por Benjamin ("va como la seda, todo perfecto") ANTES de publicar. La trae el auto-actualizador de rutina. MÓDULO TRABAJOS: Bloques: (1) FAC-MERGE `5baf17a8` — bug "agrupar trabajos en una línea" al crear factura: el diálogo de importar trabajos pendientes era aditivo/no idempotente (radio por defecto "una línea por trabajo" DEBAJO de la tabla + los ya importados se re-ofrecían hasta guardar + la línea vacía inicial bloqueaba el guardado y empujaba a reimportar) → la factura acababa con N líneas sueltas + 1 agrupada con el total. Fix: dedupe de trabajos ya importados, retirar líneas vacías al importar, radios ARRIBA (Slice 3V), IVA/retención del cliente en las líneas importadas (antes 21/0 hardcodeado). Diagnóstico con 2 agentes Explore en paralelo, convergieron en InvoiceEditorScreen:1433-1450; el camino Trabajos→Facturar (billSelected) se verificó LIMPIO en ejecución (sandbox 8081: merge=true → 1 línea). (2) TRB-DUP+TRB-MULTI+TRB-TOTAL `cedb38fc` — botón Duplicar (formulario prellenado → crea NUEVO en DRAFT, vale para BILLED, cambias fecha/cliente/lo que sea), "Repetir hasta" + "Saltar sábados y domingos" al crear/duplicar (un trabajo por día, máx 92; ráfaga de 5 creates verificada contra sandbox), y total+nº de trabajos del listado filtrado junto a las acciones. i18n 6 claves ES+EN. Merged a develop `9f5235ae`. VERIFICADO en ejecución: backend (curl sandbox benjagest_v8test/8081); el smoke de la UI JavaFX (importar trabajos al editor, Duplicar, Repetir hasta, total) SOLO puede hacerlo Benjamin → hacerlo ANTES de release. NOTA: la sesión empezó auditando por error el módulo daily-work de CONSTRUGEST (los "trabajos" eran los de Benjagest); la auditoría de Construgest quedó hecha pero sin implementar nada allí.**
>
> ## 🚀 PROMPT PARA LA PRÓXIMA SESIÓN (copiar y pegar)
>
> *"Lee la cabecera de docs/backlog.md. Última release: **v0.1.54** (VF-QR-TOGGLE: emitir facturas sin QR hasta la fecha de obligación por NIF; e2e verificado, falta solo que Benjamin vea la casilla en Facturación→Configuración). RUMBO NUEVO 2026-08-07: Benjamin NO termina Benjagest como producto — en el año nuevo empieza una app de pago; Benjagest queda para SU contabilidad. Priorizar mantenimiento/estabilidad sobre features nuevas. Anterior: v0.1.53 (módulo Trabajos: fix agrupar + Duplicar + Repetir-hasta + total; smoke verificado por Benjamin). Nada a medias de esas sesiones. Pendientes que siguen vivos: **F2R-SS** (base de cotización exacta del mes de cese con permisos sin sueldo) y los menores heredados (cabos de UI del 17-jul, 130→550 en cierre, VERI\*FACTU para 1-jul-2027 con la decisión DELEGAR vs CONSTRUIR antes). Ciclo de release (rutina): bump `APP_VERSION` en `UpdateService.java` + `build-msi.ps1 -Version 0.X.Y` (`-Rebuild` solo si cambió el backend; WiX no está en PATH) + `gh release create v0.X.Y --target <SHA completo de develop> ... dist\msi\BENJAGEST-0.X.Y.msi`. OJO: si Benjamin habla del 'módulo trabajos' es el de BENJAGEST, no el daily-work de Construgest."*
>
> **Sesión anterior 2026-07-19 (CIERRE) — ✅ v0.1.52 PUBLICADA (Latest, MSI 302 MB, target = SHA develop `d346ffa3`), ACTUALIZADA y verificada en ejecución por Benjamin. Bloques: (1) LIQ-111 — Modelo 111 (retenciones IRPF): `AeatExtraModelsService.generate111` calcula casillas desde las nóminas del trimestre + `TaxLedgerService.onPaid` asiento `Debe 4751/475 → Haber 572` + backfill + editor en TaxScreen; verificado con `Model111IntegrationTest` (2T real de Benjamin: 1 perceptor / 1.503,48 / 30,07; el pago cae a la 475 si no hay subcuenta 4751). (2) LIQ-SS — pago mensual de la Seguridad Social: `SocialSecurityLedgerService.payMonth` asiento `Debe 476 → Haber 572` por la cuota del mes (empresa+trabajador), idempotente por `SS-AAAA-MM`; botón 'Seguros Sociales' en Nómina; IGNORA cotizaciones HUÉRFANAS (sin nómina viva) y borrar nómina limpia sus cotizaciones DRAFT. (3) FIX contable de nómina (640 inflado): borrar/recalcular una nómina VALIDADA duplicaba el devengo — CAUSA RAÍZ = el contraasiento reventaba SIEMPRE (`jdbcTemplate.query(sql, rs->{...return null;})` = ResultSetExtractor que no avanza cursor → 'wrong row position') Y `PayslipService.delete` se tragaba la excepción; ahora `reverseOrContraBySource` copia líneas con queryForList + es IDEMPOTENTE (no re-contra), `createAccrual` lo usa (contrarresta el POSTED en vez de duplicar), y `delete` propaga (aborta si falla). Diagnóstico con agentes paralelos + verificado con `PayslipReversalIntegrationTest`. (4) Libro Mayor — filtro de cuenta USABLE con teclado (flechas + Enter) con Popup PROPIO (el ComboBox editable de JavaFX petaba, bug JDK-8145517), AUTO-ejecución (quitado el botón 'Ver'), botón 'Limpiar filtro'. (5) F2R — finiquito en DOS recibos (mensual prorrateada + liquidación) como la asesoría (ver `docs/design-finiquito-dos-recibos.md` y bloque en `project_benjagest_payroll_nom` de memoria). PENDIENTE **F2R-SS** = base de cotización EXACTA del mes de cese con PERMISOS SIN SUELDO (base > devengo porque el permiso baja devengo pero mantiene base); necesita el salario de CONVENIO de mes completo de Rubén (no sale limpio de las nóminas prorrateadas). La estructura de dos recibos ya funciona. Sesión larga, TODO lo publicado verificado en ejecución. SESIÓN CERRADA.**
>
> **Sesión anterior 2026-07-18 (CIERRE) — ✅ v0.1.50 PUBLICADA y ✅ v0.1.51 PUBLICADA el mismo día (Latest, MSI 302 MB, target = SHA develop). Sesión larga y muy productiva, TODO verificado en ejecución por Benjamin. Bloques: (1) MULTI-MONITOR — causa raíz = monitores con distinta escala Windows (125% + 100%): `getScreensForRectangle` mapea mal, había que aplicar coords TAL CUAL (como VS Code/WhatsApp) y buscar pantalla por `bounds.contains`. La app reabre donde se cerró (`WindowGeometry`, guardado ANTES de `System.exit` y en la X), diálogos/bloqueo/salvapantallas en el monitor de la app (vigilante global `Dialogs.installGlobalPositioner` con reintento runLater), gestor-navegador abre MAXIMIZADO en el monitor de la app (proceso aparte → se le pasa `--anchor=cx,cy`; maximizado para que la barra de título no quede fuera del borde del 2º monitor, que está en y=-104). (2) CAMPANA — fix 403 al pulsarla dentro de un cliente (`sendAsOwner`), enlace BOE clicable, nombre de cliente + fecha legible + botón Abrir cliente; Buzón DEHú retirado del sidebar. (3) 303→PAGADO — `saveFiling` no reenvía casillas si la declaración ya está PRESENTED/PAID (evita falso CONFLICT). (4) GESTOR — descargas PDF (CefDownloadHandler) + popups gestionados (onBeforePopup cancela el nativo → no cierra la app al maximizar). (5) CERT — no borrar el .p12 propio del usuario del almacén Windows (detecta "ya estaba"); marca los importados + barrido al arrancar. (6) SYS-INFO — título muestra PRUEBA·3307 SOLO en dev; en producción título "BENJAGEST" y pie LIMPIO (ni BENJAGEST ni "Backend Java + MariaDB"). (7) EQUIPOS-PIN — Config→Sesión lista y revoca los equipos multi-puesto (backend ya existía). (8) POR-VALIDAR — botón Descartar (anula draft duplicado + RefreshBus). SESIÓN CERRADA.**
>
> ## 🚀 PROMPT PARA LA PRÓXIMA SESIÓN (copiar y pegar)
>
> *"Lee la cabecera de docs/backlog.md. Última release e (auto)instalada: **v0.1.52** (Modelo 111 contable + pago SS mensual + finiquito en 2 recibos + fixes: borrar/recalcular nómina no duplica el 640, Libro Mayor con teclado/auto/limpiar, huérfanos de SS). Nada a medias, salvo **F2R-SS** (base de cotización exacta del mes de cese con permisos sin sueldo — necesita el salario de convenio de mes completo de Rubén; la estructura de dos recibos ya funciona). Ciclo de release (rutina): bump `APP_VERSION` en `UpdateService.java` + `build-msi.ps1 -Version 0.X.Y` (USAR `-Rebuild` si cambió el backend; WiX no está en PATH, añadir `C:\Program Files (x86)\WiX Toolset v3.14\bin`) + `gh release create v0.X.Y --target <SHA completo de develop> ... dist\msi\BENJAGEST-0.X.Y.msi`.*
>
...
>
> ***ACCIÓN MANUAL de Benjamin tras la 0.1.51**: reimportar SU .p12 propio al almacén de Windows (Usuario actual → Personal). La 0.1.49/50 vieja se lo había borrado; desde la 0.1.50 la app detecta "ya estaba" y NO lo toca, así que una vez reimportado se queda. Sin esto, Chrome no lo ofrece en la AEAT.*
>
> ***CABO SUELTO multi-monitor (menor)**: el vigilante global recoloca Stages JavaFX; los POPUP del gestor-navegador (window.open) abren con `setLocationByPlatform` — si algún día molesta, pasarles también el anchor. El frame principal del gestor ya abre maximizado en el monitor de la app.*
>
...
> ## 🚀 PROMPT PARA LA PRÓXIMA SESIÓN (copiar y pegar)
>
> *"Lee la cabecera de docs/backlog.md y las secciones SESIÓN 2026-07-16 (noche, 4 y 5). Última release e INSTALADA: **v0.1.48** (auto-actualizador funcionando de rutina). Todo lo del 16-jul cerrado y verificado, incluido GESTOR-CLOSE (Benjamin confirmó que va).*
>
> ***EL AUTO-ACTUALIZADOR YA ES RUTINA**: publicar release → la app de Benjamin la trae sola, en silencio, sin UAC. Ciclo: cambios → bump APP_VERSION + build-msi.ps1 -Rebuild + `gh release create v0.X --target <SHA completo de develop> ... dist\msi\BENJAGEST-0.X.msi` → Benjamin cierra y abre la app.*
>
> ***ESTADO**: Benjamin va a EMPEZAR A FACTURAR con BENJAGEST (autónomo, USO PROPIO, no la vende — ver [[project_benjagest_uso_propio]]). Módulo facturación revisado y con redes (FAC-1/2/3). Recordarle el flujo de DOS pasos: validar factura → validar el asiento en el DIARIO (si no, el IVA no entra en el 303). Y el checklist pre-factura (NIF, serie {CODE}-{YYYY}-{0000}, textos legales, tipos de IVA, modalidad NO_VERIFACTU).*
>
> ***LO PROBABLE que traiga**: (a) el resultado de su 1ª factura real (si algo falla, mirar la sección noche-4); o (b) arrancar VERI\*FACTU real, único legal pendiente (1-jul-2027, autónomo). ANTES de VERI\*FACTU releer el RE-ENCUADRE de plan-legal-2035.md y decidir CONSTRUIR vs DELEGAR la emisión (recomendado DELEGAR salvo que el objetivo sea aprender). Auditoría del código VF3 en ese doc: huella y XML hechos pero el XML oficial al XSD está HUÉRFANO (no cableado al envío), y el envío NUNCA se probó contra AEAT.*
>
> ***PENDIENTES menores (no urgen)**: 130 saldar 473 contra 550 en el cierre; duplicados de banco ANTERIORES a BANK-DUP (impide los nuevos, no limpia los viejos); cert de Susana CADUCADO en el almacén de Windows (no afecta al gestor; renovar con FNMT si lo usan).*
>
> ***Entorno e2e** (montado y probado el 16-jul): Docker `benjagest-mariadb` (3307) → BD `benjagest_v8test` → backend 8081 con `BENJAGEST_DB_URL`+`BENJAGEST_BACKEND_PORT`. Usuario `e2e-asi4@example.test` / `PruebaE2E2026$`. Factura por API: invoiceType=`NORMAL`, `POST /api/billing/invoices` + `/{id}/validate`; el asiento nace DRAFT (validar en el Diario aparte)."*
>
> **Sesión 2026-07-16 (noche, 4) — REVISIÓN del módulo FACTURACIÓN antes de la 1ª factura real (FAC-1/2/3, SIN release).**
>
> Benjamin: revisar el flujo completo (config → validar) para que la primera factura no dé errores; al validar no hay vuelta atrás y la BD no se corrige. Revisión con 2 agentes Explore en paralelo + **verificación EN EJECUCIÓN en el sandbox** (creé y validé facturas de verdad). **El flujo feliz FUNCIONA** (crear NORMAL → validar → número bien formado + huella SIF + evento SIF + asiento). Default NO_VERIFACTU+TEST (no envía a AEAT); certificado no necesario en NO_VERIFACTU; numeración atómica e idempotente. Gotchas del flujo: invoiceType válido = `NORMAL` (no F1 ni STANDARD; el CHECK `ck_sales_invoices_type` permite NORMAL/PROFORMA/RECTIFYING/SIMPLIFIED/TEST/HISTORICAL); `findActiveByKind` mapea NORMAL→serie STANDARD.
>
> 📌 **FLUJO DE DOS PASOS (diseño, hay que SABERLO)**: validar la factura NO postea el asiento — el asiento nace **DRAFT (auto-propuesto)** y hay que validarlo en el **Diario** para que pase a POSTED. **El IVA NO cuenta en el 303 hasta postear el asiento** (verificado: el 477 en DRAFT no lo cuenta el 303, que lee POSTED). Benjamin debe incorporar: factura → validar factura → Diario → validar asiento.
>
> - `[x]` **FAC-1** (`23b79d19`) — la plantilla de serie no se validaba: `{NNNN}` u otro marcador inventado se guardaba literal y la 2ª factura chocaba la UK (sobre BD que no se corrige). `SeriesService.requireValidTemplate` rechaza al crear/editar: marcadores desconocidos y plantillas sin hueco de número. Solo valen {CODE}, {YYYY}, {0000}. **Verificado**: {NNNN}→422 claro, sin número→422, {0000}→201.
> - `[x]` **FAC-2** (`23b79d19`) — sin NIF de empresa, validar reventaba a mitad con 500 genérico (dentro de la huella/evento SIF). Pre-check temprano en `validateInternal` con mensaje claro ("Ponlo en Configuración → Empresa") antes de escribir nada. **Verificado**: sin NIF→422 claro sin consumir número; con NIF→200.
> - `[x]` **FAC-3** (`0c67bb37`) — **la preocupación de fondo de Benjamin**. Al validar se escriben 2 rastros inmutables (huella VeriFactu + evento SIF). El evento SIF va en REQUIRES_NEW por buena razón (encadenamiento bajo lock, evita cadenas rotas con el scheduler) → **NO se toca**. El riesgo real: `createForSales` (asiento) era REQUIRED, y su fallo real (constraint/deadlock) marcaba la tx compartida rollback-only → reventaba la validación Y dejaba el evento SIF HUÉRFANO. **Fix: asiento → REQUIRES_NEW** (su propia tx): un fallo del asiento revierte solo su tx, la factura se valida con las cadenas consistentes, el asiento (DRAFT, regenerable) no se crea. **Verificado con un trigger que fuerza el fallo del asiento**: factura VALIDATED + huella PRESENTE + evento SIF PRESENTE + asiento AUSENTE (no a medias); happy path intacto. Residuo teórico (corte de corriente en el instante exacto entre los dos commits) no eliminable sin 2PC, desproporcionado. Razonamiento con Benjamin sobre "journal de recuperación": lo cubre InnoDB (transaccional); su idea se reserva para el envío a AEAT futuro.
> - 📌 **Checklist pre-1ª-factura para Benjamin** (de la revisión): Configuración→Empresa con **NIF** (crítico) + dirección; Facturación→Config con **modalidad NO_VERIFACTU** (default) + serie STANDARD con plantilla **{CODE}-{YYYY}-{0000}** + textos legales + régimen IVA + tipos de IVA; pulsar Guardar en CADA bloque. Y recordar el 2º paso: validar el asiento en el Diario.
>
> **[HIST cerrado] Sesión 2026-07-16 (noche, 3) — GESTOR-NAVEGADOR completo: cert OK + fix parser (0.1.47) + GESTOR-CLOSE (sin release).**
>
...
> - `[x]` **GESTOR-CLOSE** (en v0.1.48) — al salir del cliente vinculado, la ventana del gestor se quedaba abierta con la sesión de ESE cliente. Fix: campo `gestorProcess` + `exitClientMode()` → `closeGestorNavegador()` mata el proceso y sus descendientes (subprocesos de Chromium). `exitClientMode` es el punto central de salida (atrás/dashboard/módulos). Al morir el proceso, el Task despierta y quita la huella del almacén (el cert del cliente tampoco queda residente). **Verificado**: mecanismo `descendants().forEach(destroy)+destroy()` mata un proceso con 3 subprocesos, 0 huérfanos; y **✅ VERIFICADO VISUALMENTE por Benjamin el 16-jul ("la pantalla navegador va ok")**. Cerrado.
>
...
> - `[x]` **✅ EL AUTO-ACTUALIZADOR FUNCIONÓ POR FIN** (0.1.45→0.1.46, primera vez real con el guard UPD-3c arreglado): Benjamin *"la actualización de lujo, no sabía que la habíamos hecho completamente en silencio, todo en segundo plano y al final se reinicia benjagest"*. El `update.log` lo confirma: esperó a la app, paró servicio (PID), msiexec código 0, modules 119,3 MB presente, servicio Running. SIN UAC, SIN ventanas.
> - `[x]` **BUG que introdujo BROWSER-CERT-STORE-FIX y su fix (v0.1.47)** — Benjamin: en "mi gestión" el navegador ABRE, en cliente vinculado NO ("No se pudo abrir el gestor navegador"). **CAUSA (reproducida)**: en modo cliente el cert existe → `/browser/material` devuelve el .p12 en base64 (~9 KB) → la UI lo parseaba con `textField`, cuya regex hace catastrophic backtracking sobre 9 KB → **StackOverflowError** (un Error, NO Exception → el `catch(Exception)` no lo pillaba → el Task moría → gestor.launch.fail). En "mi gestión" (empresa propia sin cert) el endpoint da 204, no hay base64, y abría. Es la deuda del parser JSON de la UI (§6 CLAUDE.md). **FIX**: `AltaApiClient.rawStringField` (indexOf, sin regex) para el base64 + el catch de la preparación del cert pasa a `Throwable` (best-effort: nada impide abrir el navegador). Verificado: textField sobre 9 KB → StackOverflow; rawStringField → 9003 chars exactos. MSI 0.1.47 publicado. **PENDIENTE**: prueba visual de Benjamin en modo cliente (abre Y ofrece el cert).
>
...
> ## 🚀 PROMPT PARA LA PRÓXIMA SESIÓN (copiar y pegar)
>
> *"Lee la cabecera de docs/backlog.md y la sección SESIÓN 2026-07-16 (noche). MSI **0.1.45** en `dist\msi\BENJAGEST-0.1.45.msi`, verificado por extracción (JRE 119,3 MB + los 5 bloques del día + `runningAsService`=true con el fat jar real). Benjamin lo tiene para instalar A MANO.*
>
> ***LO PRIMERO: ¿instaló la 0.1.45 a mano?** (parar servicio → MSI → arrancar). Si NO, es lo primero. Si SÍ → verificar jar 0.1.45 + modules 119,3 + servicio + health + endpoint 403, y probar VISUALMENTE los 3 fixes (extracto no duplica, Regularizar crea pago 130/303, y de paso que la app arranca bien).*
>
> ***LUEGO: publicar la 0.1.45** en GitHub (`gh release create v0.1.45 --target <SHA completo de develop> ... dist\msi\BENJAGEST-0.1.45.msi`). SOLO después de que la 0.1.45 esté instalada a mano — si se publica antes, la 0.1.43 instalada (guard roto) le ofrecerá update y fallará. Al publicar, la 0.1.45→0.1.46 será por FIN la primera prueba REAL del auto-actualizador CON el guard bueno.*
>
> ***OJO — la lección de hoy**: el auto-actualizador NUNCA había funcionado en producción. El guard se probó rechazando un backend de desarrollo, nunca aceptando el servicio real; getCodeSource() funciona en un jar simple pero PETA en el fat jar de Spring Boot (classloader anidado, mensaje null). Fix UPD-3c: resolver por `java.class.path` (con `java -jar X.jar` es "X.jar"). Verificado con `-XshowSettings` sobre el fat jar REAL + integración `java -jar` (true con BENJAGEST.exe al lado, false sin él). NO fiarse de getCodeSource en un fat jar.*
>
> ***PENDIENTE del 130** (otro slice): saldar la 473 contra el titular (550) en el cierre. **DUPLICADOS VIEJOS del banco**: BANK-DUP impide los nuevos, no limpia los que ya tenga. **Verificación VISUAL de Benjamin** de los 3 fixes sigue pendiente.*
>
> ***Entorno e2e**: Docker `benjagest-mariadb` (3307) → `benjagest_v8test` → backend 8081 con `BENJAGEST_DB_URL`+`BENJAGEST_BACKEND_PORT`. Usuario `e2e-asi4@example.test` / `PruebaE2E2026$`. Tablas `journal_entries`/`journal_entry_lines` (FK `journal_entry_id`). Compra `/api/purchases/invoices` (+`/{id}/pay`=GAS-2); bancos `/api/accounting/bank-movements` (+`/{id}/link`,`/match-suggestions`,`/reconcile-review`); cuenta bancaria necesita `accountingAccountId`."*
>
> **Sesión 2026-07-16 (noche) — el auto-actualizador falló y se arregló el guard (UPD-3c) + 0.1.45.**
>
> - `[x]` **Se publicó la 0.1.44 y Benjamin la disparó** (había elegido "actualizar sola"). **FALLÓ**: diálogo "No se pudo actualizar / Este backend no es el servicio instalado". El `update.log` NO se creó → el guard cortó ANTES del helper (nada de la instalación se tocó, bien).
> - `[x]` **UPD-3c** (`71d43eb2`) — **causa raíz del log del servicio, no deducida**: `UPD-3: no pude saber si soy el servicio (null); me niego por si acaso.` El `(null)` = excepción en `runningAsService()`: `getCodeSource().getLocation().toURI()` PETA dentro del fat jar de Spring Boot (classloader anidado, la location no es un `file:` normal). `installDir()` igual (caía al default hardcodeado, correcto para Benjamin por casualidad). **Un jar simple SÍ funciona con getCodeSource; el fat jar NO** → no saltó hasta producción. Es EXACTO lo que avisaba el backlog: el guard se probó rechazando dev, nunca aceptando el servicio real. **Fix**: resolver la ruta por `java.class.path` (núcleo `jarLocation(cp,userDir)` PURO). **Verificado en ejecución**: `-XshowSettings` sobre el fat jar REAL → `java.class.path=backend-java-...jar` (el launcher no lo altera); integración `java -jar backend.jar` → `runningAsService` TRUE con BENJAGEST.exe al lado, FALSE sin él (el caso que fallaba, ahora pasa); 5 tests nuevos de jarLocation. Suite **155 verde**.
> - `[x]` **Bump 0.1.45** (`aac820e1`) + MSI reconstruido — número nuevo para no tener dos 0.1.44 distintas. MSI verificado: JRE 119,3 MB, APP_VERSION 0.1.45, BENJAGEST.exe junto al jar, `java.class.path=backend.jar`, y los 5 bloques dentro (UPD-3c/UPD-CLEAN/BANK-DUP/LIQ-130-BF/UPD-3b). UPD-CLEAN funcionando: al construir dejó 0.1.44+0.1.45, borró 0.1.43.
> - `[x]` **Release 0.1.44 BORRADA de GitHub** (`gh release delete --cleanup-tag`, con OK de Benjamin) — estaba rota (guard) y nadie la instaló. Latest vuelve a **0.1.42** < 0.1.43 instalada → la app deja de ofrecer updates que fallan. La 0.1.45 se publicará DESPUÉS de instalarla a mano.
> - `[x]` **0.1.45 instalada a mano y verificada** (jar 0.1.45, JRE 119,3 MB, guard `jarLocation` presente, servicio Running, health 200, endpoint 403) + **PUBLICADA** en GitHub (latest = v0.1.45; su app no se auto-ofrece nada, misma versión).
> - `[x]` **Pregunta de Benjamin sobre Regularizar (NO es bug, verificado e2e)**: "me sale el 303 y el 130 del 1T; si el 303 ya lo regularicé, ¿no comprueba si ya existe el asiento?". **SÍ comprueba** (`existingEntry` por company+source_type+source_id, excluye VOIDED). Lo que le sale del 303 **no es la liquidación otra vez, es el PAGO** (asiento distinto): la versión vieja de Regularizar solo creaba la liquidación (vacía la 477); el pago (Debe 4750/Haber 572, saca el dinero del banco) nunca se creó = el agujero 2 de LIQ-130-BF. Reproducido en sandbox: 303 1T PAID con liquidación → la vista previa devuelve SOLO `kind=PAYMENT` (la liquidación NO se re-ofrece); aplicar → sigue habiendo **1 liquidación** (no duplica) + nace **1 pago**. La columna "Asiento" de la tabla lo dice (pone "Pago"). **Posible mejora UI**: que la confusión de Benjamin indica que "Pago" vs "Liquidación IVA" no se lee claro — pendiente de si lo pide.
> - 📌 **El auto-actualizador (UPD-3) nunca funcionó en producción hasta hoy**. Toda la cadena (helper sobrevive, WinSW no lo mata, vigía fuera de la carpeta) estaba bien MENOS el guard de entrada, que rechazaba al propio servicio. Ahora sí debería; se prueba de verdad en 0.1.45→0.1.46.
>
> **[HIST cerrado] 2026-07-16 tarde — Benjamin instaló la 0.1.43 a mano y verificada (jar 0.1.43, JRE 119,3 MB, registro y disco cuadran, endpoint UPD-3 vivo 403). Luego 3 bugs suyos, TODOS cerrados y verificados e2e + MSI 0.1.43 RECONSTRUIDO con los 3 dentro: BANK-DUP (el extracto duplicaba el gasto del autónomo) + LIQ-130-BF (Regularizar no creaba el pago del 130 NI del 303) + UPD-CLEAN (retención de versiones app + repo). Suite 150 verde. **RELEASE SIGUE SIN PUBLICAR**. Detalle abajo.**
>
> **[HIST cerrado] 2026-07-16 mañana — UPD-3 + UPD-3b mergeados, 130 e2e, primer MSI 0.1.43.**
>
> ## 🚀 PROMPT PARA LA PRÓXIMA SESIÓN (copiar y pegar)
>
> *"Lee la cabecera de docs/backlog.md y la sección de la SESIÓN 2026-07-16 (tarde). Benjamin YA tiene la **0.1.43** instalada y verificada. En `dist\msi\BENJAGEST-0.1.43.msi` hay un MSI RECONSTRUIDO por la tarde que además lleva BANK-DUP + LIQ-130-BF + UPD-CLEAN dentro (verificado por extracción: los 4 bloques + JRE 119,3 MB).*
>
> ***OJO — hay DOS 0.1.43**: la que Benjamin instaló por la mañana (solo UPD-3/UPD-3b) y la reconstruida por la tarde (con los 3 fixes nuevos). MISMO número de versión. Si quiere los fixes del extracto/130/limpieza en su app instalada, tiene que REINSTALAR a mano el MSI de la tarde (parar servicio → MSI → arrancar), porque el número no cambió y el actualizador no lo ve como "más nuevo". Preguntarle si reinstala ya o si esos fixes esperan a la 0.1.44.*
>
> ***PENDIENTE de verificación VISUAL de Benjamin** (no se pueden pulsar botones JavaFX desde la sesión): (1) importar un extracto con un gasto ya pagado por "Registrar pago" → que NO lo premarque y muestre el pago como prueba; (2) Bancos → conciliar dos veces el mismo movimiento → un solo asiento; (3) Impuestos → Regularizar trimestres → que salgan los pagos del 130 y 303 con su fecha de plazo.*
>
> ***RELEASE: sigue SIN PUBLICAR, a propósito.** La 0.1.42→cualquiera y la 0.1.43 instalada comprueban updates solas al arrancar (`enterApp → checkForUpdates`) con el actualizador VIEJO. Publicar en GitHub le ofrece la actualización con el flujo que rompió la app 3 veces. Orden: primero que la 0.1.43-tarde esté instalada a mano y probada; LUEGO se publica; y la 0.1.43→0.1.44 es la primera prueba REAL del actualizador nuevo (UPD-3), con log en `%ProgramData%\BENJAGEST\logs\update.log`. UPD-CLEAN ya versiona el MSI descargado y conserva 2 anteriores.*
>
> ***PENDIENTE del 130 (otro slice)**: saldar la 473 contra la cuenta del titular (550) en el cierre — el IRPF de un autónomo es personal, no gasto de la actividad. La 473 ya acumula bien.*
>
> ***DUPLICADOS VIEJOS**: BANK-DUP impide los NUEVOS, no limpia los que Benjamin ya tenga en sus libros de conciliaciones anteriores. Si los hay, hay que listarlos y decidir (ojo trimestres ya presentados).*
>
> ***DEUDA**: `TaxLedgerService` (303+130) ya tiene su primer test hoy en espíritu (LIQ verificado e2e) pero SIGUE sin test unitario propio; sí lo tiene ya `SystemUpdateService` (UPD-CLEAN, 8 tests). Montar harness Spring+BD para el ledger fiscal es un slice.*
>
> ***Entorno e2e** (probado el 16-jul): Docker `benjagest-mariadb` (3307) → BD `benjagest_v8test` → backend 8081 con `BENJAGEST_DB_URL`+`BENJAGEST_BACKEND_PORT`. Usuario `e2e-asi4@example.test` / `PruebaE2E2026$`. **Nombres de tabla**: `journal_entries` / `journal_entry_lines` (FK `journal_entry_id`); compra: `/api/purchases/invoices` (POST crear, POST `/{id}/pay` = GAS-2); bancos: `/api/accounting/bank-movements` (+`/{id}/link`, `/match-suggestions`, `/reconcile-review`). Cuenta bancaria necesita `accountingAccountId` (una 572). `tax`+`accounting` activos."*
>
> **Sesión 2026-07-16 (tarde) — 3 bugs de Benjamin: BANK-DUP + LIQ-130-BF + UPD-CLEAN.**
>
> Benjamin instaló la 0.1.43 y reportó 3 cosas: (1) al importar extracto para conciliar, el gasto del autónomo se DUPLICA (anulaba uno a uno); (2) el 130 no responde al regularizar trimestres (no crea el asiento del pago); (3) petición: al actualizar, borrar versiones obsoletas dejando solo 2 anteriores.
>
> - `[x]` **Verificación de la 0.1.43 instalada** — jar en disco 0.1.43 (leído DENTRO del jar, no del registro), `runtime\lib\modules` 119,3 MB, servicio Running, health 200, registro y disco CUADRAN (el 15-jul no), endpoint `/api/system/update/status` → 403 (vivo y protegido). Cola de borrado: 28 entradas pero TODAS `Config.Msi\*.rbf` (basura de msiexec), 0 de la app.
> - `[x]` **BANK-DUP** (`cee87cac`) — diagnóstico con 2 agentes Explore en paralelo (§2), convergieron en `BankMovementService`. **Causa**: nadie leía `purchase_invoices.paid` (la columna que V167 añadió "para impedir pagar dos veces") en NINGUNO de los 3 sitios que deciden "ya pagado". Y "Registrar pago" (GAS-2) escribe SOLO ahí, no en las tablas que la conciliación miraba. El script `fix-duplicate-ss-autonomo.sql` del 9-jul MANDÓ pagar 6 cuotas SS con GAS-2 → 6 duplicados al importar (**un arreglo fabricó el bug siguiente**). 4 slices: BANK-DUP-1 (`suggestMatches` PURCHASE no filtraba `paid=FALSE` ni `status<>VOID` → auto-conciliación creaba el duplicado SOLA al importar, era el grave), -2 (`findBestCandidate` pasaba `null` como paymentStatus → checkbox premarcado; el comentario "V39/V40 no tiene payment_status" era falso desde V167 — §10.bis), -3 (`existingPaymentsFor` sumó la 4ª vía GAS-2 → la pantalla enseña el pago como prueba, lo que su texto ya prometía), -4 (idempotencia en `linkToInvoice`: si el movimiento ya tiene asiento, no crea otro; no hay UNIQUE en `(source_type,source_id)`). i18n: paySource pasó de español crudo a CLAVE+par ES/EN. **Verificado e2e** replicando su caso exacto (cuota TGSS → Registrar pago → cargo en extracto): match-suggestions [] (no auto-concilia), reconcile-review SETTLED con el pago visible, CONTROL de que una factura SIN pagar SÍ se propone (no apagué la conciliación), 3 links → 1 asiento. Ver [[project_benjagest_pago_multiflujo]].
> - `[x]` **LIQ-130-BF** (`b06fdce9`) — tenía razón, y había un 2º agujero peor que no vio. AGUJERO 1: `previewPendingLiquidations` y `applyBackfill` llevaban `tax_model_code='303'` CLAVADO → el 130 no aparecía jamás. AGUJERO 2: el backfill solo llamaba a `createLiquidation303`, **el asiento de PAGO no lo creaba NUNCA, tampoco para el 303** → un 303 ya pagado regularizado dejaba la 4750 debiendo PARA SIEMPRE dinero ya pagado (mismo agujero que la 477, una capa abajo). Por qué mi verificación de la mañana dio verde: yo probé "marcar PAGADO ahora" (dispara `onPaid`); él tiene declaraciones ya pagadas de antes, donde solo el backfill las alcanza. **Fecha del pago** (decisión Benjamin): el PLAZO del modelo (1T 20-abr…4T 30-ene), no hoy — hoy metería el pago del 1T en el trimestre del botón. Reusa `deadline_at` guardado y si falta `TaxFilingService.computeDeadline` (hecho static, sin duplicar plazos). Record pasa a `(filingId, kind)` — un trimestre da 2 filas (liquidación+pago). UI: columnas Modelo/Asiento/Fecha; textos header/note/summary REESCRITOS (mentían: hablaban solo de IVA/477). **Verificado e2e**: 130+303 pagados sin asiento, deadline_at NULL (para ejercitar el fallback) → previa muestra los 2 pagos fechados 20/07 (plazo del 2T), aplicar → 473/572 400 y 4750/572 150 cuadrados y fechados 20/07 no hoy, idempotente (id repetido → created:0), 473 acumula 970=570+400.
> - `[x]` **UPD-CLEAN** (`f9705d2f`) — retención de versiones, 2 frentes (releases de GitHub NO se tocan, decisión suya). **App** (`SystemUpdateService`): el MSI descargado se guarda con su versión en el nombre (antes `BENJAGEST-update.msi` fijo, se pisaban → sin histórico) y `pruneOldInstallers` deja 3 (nuevo+2), barre el `update.msi` sin versión del flujo viejo, nunca borra el que se instala, orden NUMÉRICO (0.1.9<0.1.10). **Repo** (`build-msi.ps1`): `dist\msi` de 2,9 GB → 0,9 GB, deja 2 por variante (advisory/puesto conviven, prefijo anclado); y DE PASO se arregló el mensaje final que MENTÍA (mostraba "el primer .msi que haya"). **8 tests nuevos** (`SystemUpdateServiceTest`) — 2 fallaron primero y destaparon un fallo real (el current contaba fuera del cupo → conservaba 4 no 3); corregido. Purga del repo probada en COPIA antes de tocar `dist\msi`; build real 2,9→0,9 GB.
> - `[x]` **MSI 0.1.43 RECONSTRUIDO** — `build-msi.ps1 -Rebuild` con los 3 fixes. Verificado por extracción: los 4 bloques del día dentro (SystemUpdateService, `BANK-DUP` en BankMovementService, `backfillPaymentDate` en TaxLedgerService, `neutralWorkingDir` en UI) + JRE 119,3 MB + su java arranca. **NO publicado.** Suite **150 verde**.
>
> **[HIST cerrado] Sesión 2026-07-16 (mañana) — UPD-3b + merge de UPD-3 + el 130 + primer MSI 0.1.43.**
>
> - `[x]` **UPD-3b** (`0b689a02`) — **encontrado revisando UPD-3 antes de mergearlo, no por un fallo**. `spawnRelauncher` lanzaba el vigía con `ProcessBuilder` sin `directory()` → **heredaba el cwd de la app**, y el `.lnk` del menú inicio la lanza con `WorkingDirectory=C:\Program Files\BENJAGEST` (leído del acceso directo, no supuesto). O sea: el vigía se plantaba **dentro de la carpeta que msiexec reemplaza**, durante los 20 min de espera. El javadoc presumía LITERALMENTE de lo contrario ("no toca la carpeta de instalación, no bloquea nada"). **Medido antes de alarmar**: un proceso con el cwd ahí **NO** impide reemplazar ficheros ni borrar subcarpetas — solo impide renombrar/borrar la raíz → **no era la causa del 3010**. Pero en este módulo no se deja al azar algo que cuesta una línea. Fix + `neutralWorkingDir` (rechaza candidatas con `startsWith(installDir)`, no `!=`). De paso fuera un **javadoc huérfano de 40 líneas** (era el del viejo `launchInstaller`, flotando encima de `spawnRelauncher` y describiendo mal el flujo justo donde entenderlo mal ya costó 3 roturas).
> - `[x]` **LA DUDA QUE TUMBABA UPD-3, RESUELTA CON UNA PRUEBA** — el helper es lanzado por el `java.exe` del servicio y lo primero que hace es **parar ese mismo servicio**. El servicio es **WinSW 2.12**, que al parar **mata a los hijos del proceso**: si el helper fuera descendiente suyo, se suicidaría a mitad de `msiexec` y dejaría la app cerrada y sin actualizar. **No se supuso: se montó con procesos de mentira, sin tocar BENJAGEST.** Resultado: el helper arranca con **PPID = el `cmd` intermedio YA MUERTO**, no el padre; el único hijo vivo del padre es un `conhost`. Al matarle el árbol al padre (como haría WinSW) **el helper SOBREVIVIÓ y siguió trabajando**. El `cmd /c start /b` desliga de verdad → da igual lo que haga WinSW al parar.
> - `[x]` **UPD-3 + UPD-3b MERGEADOS** (merge `7b92e5f8`) — ya no están "en feat/Benjamin sin mergear".
> - `[x]` **130 VERIFICADO EN EJECUCIÓN** (sandbox v8test, nada real tocado) — era lo que estaba "hecho pero sin probar":
>   - **La trampa de la 4750 NO se repite**: la **473 SÍ existe** en la plantilla permanente (`V147:83`, "HP retenciones y pagos a cuenta", ASSET). No hace falta migración.
>   - `accountByPrefix` ordena por `LENGTH(code)` → con 473 y 4730 en el plan coge la **473**. Determinista.
>   - 130 1T: 3000 ingresos → 2850 neto (5% dif. justificación) → cuota 570 → PAID → **asiento nº13 POSTED `TAX_PAYMENT`: Debe 473 570,00 / Haber 572 570,00**, cuadrado, fechado HOY (fecha de caja, por diseño).
>   - **Idempotencia**: PAID mandado ×3 más → sigue habiendo **1** asiento.
>   - **Importe 0 → NO crea asiento** (y no revienta).
>   - **El 130 es acumulativo y encadena bien**: el 3T vio `pagosPrevios: 570` (el pago del 1T) y pidió 380, no 950.
>   - **El 409 al corregir un PAID NO es un bug: es la ley.** `AUDIT-T2` (RD 1007/2023) prohíbe tocar las casillas de una declaración presentada; la vía es anular (CANCELLED) + crear la correcta. Ver [[feedback_benjagest_sif_inalterable]].
>   - **i18n OK de anoche**: `TAX_LIQUIDATION`/`TAX_PAYMENT` tienen par ES+EN (`I18n:7505/7506` y `8887/8888`) y están en el filtro Origen del Diario (`AccountingScreen:413`).
> - `[x]` **MSI 0.1.43 construido Y VERIFICADO SIN INSTALAR** (`3e834689` bump) — `dist\msi\BENJAGEST-0.1.43.msi`, 302,2 MB. Verificado con `msiexec /a` (extrae sin instalar, sin admin): **`runtime\lib\modules` = 119,3 MB PRESENTE** (lo que faltaba en la 0.1.39), **su `java.exe` arranca** (Temurin 21.0.11), el ui.jar lleva **APP_VERSION 0.1.43 + `spawnRelauncher` + `neutralWorkingDir`**, y el backend.jar lleva **`SystemUpdateController`/`SystemUpdateService`**. Suite **142 verde**.
> - ⚠️ **NO PUBLICADA la release** — a propósito, decisión de Benjamin ("adelante" al plan). Ver el aviso del prompt de arranque.
>
> 📌 **Corrección al parte de anoche**: el prompt decía "cola de borrado limpia". **No lo estaba**: había 28 entradas. Pero las 14 eran `C:\Config.Msi\*.rbf` = basura temporal de rollback de msiexec, **ninguna de la app**. Inofensivas (nada que ver con las 53 de borrado de la app del 15-jul). Reiniciar era seguro.
>
> 📌 **Nota de proceso**: hoy sí se respetó la sección 3 (todo por `feat/Benjamin` → merge `--no-ff` a develop).

> **[HIST] 2026-07-15 (noche) — ✅ v0.1.42 PUBLICADA E INSTALADA (a mano). Bloques del día: DASH (contador de cobros) + ASI (Anular / Reclasificar cuenta / 700→705) + ANUL (anular no resta dos veces; anular factura no borra su asiento → **NOTA #1 CERRADA**) + CRED-OUT (fuera la pantalla de Credenciales) + LIQ (los modelos AEAT ya generan asiento: la 477 se vacía cada trimestre) + LAUNCH-1. ⚠️ **EL ACTUALIZADOR ROMPIÓ LA INSTALACIÓN 3 VECES**; UPD-3 (por servicio, sin UAC) está **hecho y SIN MERGEAR** en `feat/Benjamin` — es lo PRIMERO de mañana. Smoke visual de Benjamin CONFIRMADO: "he podido anular el asiento, y cambiar la cuenta de la venta, todo perfecto". Prompt de arranque abajo.**
>
> **[HIST] 2026-07-13 — v0.1.37 PUBLICADA E INSTALADA (tag v0.1.37 → develop b749046a): fixes post-0.1.36 = dedup PDF por SHA (no duplica asiento/303) + i18n invoice_type (columna Tipo) + update flow (launchInstaller para el servicio antes de msiexec) + extracción venta (NIF cliente=emitterNif + concepto multilínea). Smoke dedup SHA verde (reimport→409, 1 asiento). INSTALADA por el camino limpio (parar servicio→MSI→arrancar) SIN romper el JRE (valida el fix): servicio Running/Automatic, health 200, lib\modules intacto. PENDIENTE Benjamin: smoke VISUAL en 0.1.37 (reimportar PDF venta → NIF+concepto rellenos, sin duplicado; columna Tipo = "Histórica"). Nota: nombre de cliente en facturas a 2 columnas sigue best-effort. Histórico v0.1.36 abajo.**
>
> **[HIST] 2026-07-12 — v0.1.36 PUBLICADA (tag v0.1.36, target develop d6e8735b): FAC-CLIVAL + VF3-XML + Tema 2 (factura PDF emitida en Facturación) + Tema 1 (bloque COMP- compensación, 5 slices). Smoke end-to-end VERDE (import PDF venta cuenta 1 vez en 303; compensación 400/430 + reversión). Bug de binding Spring (@RequestParam/@PathVariable sin nombre) cazado+corregido en el smoke. MSI advisory (302 MB) + puesto (71 MB) reconstruidos y adjuntos a la release. Mergeado a develop. PENDIENTE: Benjamin actualiza su app instalada a 0.1.36 (UpdateService detecta la release) → reinstalar/reiniciar el servicio; smoke VISUAL de FAC-CLIVAL y de la pestaña Compensación. Cuadre 130 RESUELTO.**
>
> ## 🚀 PROMPT PARA LA PRÓXIMA SESIÓN (copiar y pegar)
>
> *"Lee la cabecera de docs/backlog.md y la sección de la SESIÓN 2026-07-15. Última release **v0.1.42**, instalada a mano y verificada (jar en disco 0.1.42, JRE 119,3 MB, cola de borrado limpia).*
>
> ***LO PRIMERO: UPD-3.** Está commiteado en `feat/Benjamin` (`1d2915fa`) y **SIN MERGEAR a propósito**. El actualizador rompió la instalación de Benjamin TRES veces el 15-jul. Causa REAL (probada con `PendingFileRenameOperations`): quien bloqueaba los ficheros **NO era el servicio sino LA PROPIA APP** (39 jar de `app\` + `runtime\lib\modules`); todo el arreglo anterior (UPD-1) perseguía al sospechoso equivocado. UPD-3 = lo actualiza el SERVICIO (LocalSystem, ya es admin → sin UAC, y la app está muerta antes de la copia). Ya verificado: helper pasa el parser de PowerShell (575 tokens, 0 errores), deja LOG en `%ProgramData%/BENJAGEST/logs/update.log`, espera a la app POR NOMBRE, endpoint 403 sin token, ignora rutas (se baja el MSI oficial: aceptar un path sería elevación de privilegios porque el 8080 escucha en toda la LAN), y el GUARD `runningAsService()` impide que un backend de desarrollo reinstale la instalación real (salió en la primera prueba: pasó de verdad). **FALTA**: el ciclo completo de actualización, que solo se prueba sobre la instalación real. **TRAMPA**: la 0.1.42→0.1.43 la hará el actualizador VIEJO (el nuevo viaja DENTRO de la 0.1.43) → la 0.1.43 se instala A MANO y el nuevo solo se prueba en 0.1.43→0.1.44.*
>
> ***LUEGO: el 130.** `TaxLedgerService` ya genera `Debe 473 / Haber 572` al pagarlo, pero **NO está probado** (solo se verificó el 303). Benjamin **NO factura con retención** (confirmado; sus totales son base × 1,21 exacto), así que su 473 está a cero y el asiento cuadra solo — el contraasiento que él temía no hace falta. PENDIENTE de otro slice: saldar la 473 contra la cuenta del titular (550) en el cierre, porque el IRPF de un autónomo es personal, no gasto de la actividad.*
>
> ***Y DESPUÉS: release 0.1.43** con UPD-3 + el 130. Recordar el gate del CLAUDE.md 10.ter y que build-msi.ps1 MIENTE en su mensaje final (imprime "BENJAGEST Puesto-0.1.35.msi"); el bueno es `dist\msi\BENJAGEST-<version>.msi`.*
>
> ***Entorno e2e** (montado y probado el 15-jul): Docker → contenedor `benjagest-mariadb` (3307) → BD sandbox `benjagest_v8test` (el usuario `benjagest` YA tiene grants sobre v4test..v8test) → backend en 8081 con `BENJAGEST_DB_URL` + `BENJAGEST_BACKEND_PORT`. **No hay usuarios demo** (V148 los purga): usuario de pruebas ya creado `e2e-asi4@example.test` / `PruebaE2E2026$`. El campo de IVA de una línea es **`vatPercent`** (con `vatRate` la factura sale SIN IVA en silencio) y el PUT de filings exige el objeto completo. El módulo `tax` hay que activarlo por SQL. Cliente mysql en `C:\Program Files\MariaDB 12.2in\mysql.exe`.*
>
> ***Lo que NO se toca**: el descuadre histórico de las anulaciones viejas — Benjamin decidió no repararlo (se le avisó de que el -701,80 de la FRA-2026-0006 lo causó el código viejo de BENJAGEST, no su asesoría; aun así lo deja).*
>
> ***La lección del día**, que costó tres roturas: el actualizador me ganó siempre por SUPONER en vez de mirar (di por hecho que la app no se cerraba: falso; que el culpable era el servicio: falso; que un PowerShell elevado puede leer `$_.Path` de un proceso de SYSTEM: **nunca lo comprobé y era la base del arreglo**). Y lo agravé dejando el script corriendo OCULTO Y SIN LOG. Si algo de esto se vuelve a tocar: **log primero, mirar después, suponer nunca**."*
> **Sesión 2026-07-15 (bloques DASH + ASI + ANUL — 4 bugs de Benjamin, 3 bugs más cazados por el e2e). ✅ NOTA #1 CERRADA.**
>
> Benjamin reportó 4 cosas: (1) "Pendientes cobro" decía 4 con 1 sola factura pendiente; (2) el dashboard no se refrescaba al cobrar; (3) un asiento manual duplicado y validado que NO se podía eliminar; (4) una venta contabilizada en 700 (mercaderías) siendo prestación de servicios. Patrón común: **el backend ya hacía lo correcto, el UI no lo alcanzaba.**
>
> - `[x]` **DASH-1** (`b4c7738a`) — el KPI filtraba `payment_status='PENDING'` en crudo. Anuladas y rectificativas conservan un PENDING residual en BD y sumaban. Ahora usa `collectionNotApplicable()`, el predicado que YA existía y que la columna Cobro del listado usa para pintar "—" (su javadoc decía que el KPI "también los excluye": nunca lo hizo). Confirmado contra la BD dev: 0006 VOIDED + 0006R rectificativa + 0007 = los 3 PENDING que veía; ahora 1.
> - `[x]` **DASH-2** (`b4c7738a`) — el dashboard era el ÚNICO consumidor de ventas sin `RefreshBus.subscribe`. Los `emit(TOPIC_SALES)` ya existían; faltaba el receptor.
> - `[x]` **ASI-1** (`edc16914`) — `@Transactional` mal colocada (entre el javadoc de voidEntry y el de deleteImportedByIds → anotaba al de abajo). voidEntry corría en autocommit.
> - `[x]` **ASI-2** (`80b01445`) — botón "Anular" en el Diario. `AccountingApiClient.voidEntry` existía y **no lo llamaba nadie**: código muerto. Por eso "no se podía eliminar" — la capacidad existía pero era inalcanzable. (`/duplicates/delete` no servía: filtra `source_type IN (SALES_PDF_IMPORT, PURCHASE_INVOICE)` y un manual tiene NULL → se saltaba EN SILENCIO.) Editor read-only si POSTED (antes te dejaba teclear y el backend soltaba 409).
> - `[x]` **ASI-3** (`35e91eb7`) — el 700 vs 705. El classifier nunca estuvo roto: `SalesJournalEntryService:123` armaba el texto con `notes+invoiceNumber` y **se dejaba fuera `invoice.concept()`** — el campo donde el usuario escribe qué vendió. Caía al fallback 700 mientras la descripción de la línea SÍ usaba el concepto (de ahí el asiento absurdo: 700 rotulada "Prestación de servicios"). **Bug extra que cazó el test**: la regla 705 no tenía plural — "ServicioS de reparación" tampoco matcheaba. `IncomeAccountClassifierTest`, 6 verde.
> - `[x]` **ANUL-1** (`023cf39f`) — **cierra la nota #1** (línea ~827, abierta desde 2026-07-07 "revisar juntos con un caso real"). El Diario quedaba en -X porque `reverseForSales` **BORRABA** el asiento de la original y solo sobrevivía el -X de la rectificativa. **La rectificativa YA ES el contraasiento: no había que añadir nada, había que dejar de borrar.** Para ejercicio cerrado esto ya se hacía bien por accidente (el 409 se absorbía) y el comentario lo llamaba "el único camino legal" — lo era también para el abierto. Verificado e2e: factura 2000 anulada → 4300001 neto 0,00 y 759 neto 0,00, y la factura CONSERVA su asiento (1, no 0).
> - `[x]` **ANUL-2** (`ece9cf2c`) — **BUG PREEXISTENTE cazado por el e2e, no por revisión**. `voidEntry` marcaba VOIDED **y además** creaba contraasiento: como todo filtra `status='POSTED'`, el VOIDED ya deja de contar y el contraasiento restaba OTRA VEZ. Medido: anular un duplicado de 500 → 600 saldo **-500** y 400 **+500** (esperado 0/0). Nunca saltó porque ningún botón llamaba a voidEntry; ASI-2 fue el primero. **Decisión Benjamin: VOIDED = anulado, visible, no computa; sin contraasiento.** OJO: devolver el contraasiento sin quitar el VOIDED trae de vuelta el -X.
> - `[x]` **ASI-4** (`ece9cf2c` + `96efcaa4`) — `POST /accounting/journal-entries/{id}/reclassify-account` + botón "Reclasificar cuenta". Vía A3/Sage: anular + reasentar (el original a VOIDED, nace el correcto). `saveRule` crea la regla `INCOME_ACCOUNT_BY_CUSTOMER_NIF`. **Verificado e2e el círculo completo**: factura de servicios → asiento en 705 (ASI-3 real) → reclasificar a 759 con regla → la SIGUIENTE factura de ese cliente nace ya en 759.
> - `[x]` **ANUL-2b** (`96efcaa4`) — **lo destapó nuestro propio arreglo**. `nextPostedEntryNumber` hacía MAX filtrando POSTED, ignorando que un ANULADO **conserva su entry_number** → proponía un número ocupado → UK `uk_journal_entries_company_number` → **HTTP 500 al validar**. Invisible hasta ANUL-2 (no había anulados). Auditados los 8 numeradores con grep `MAX(entry_number)`: solo otro igual (`PurchaseJournalEntryService:654`), arreglado.
>
> **Riesgo que evitó un agente Explore**: el diseño inicial de ASI-4 rompía 2 cosas en silencio, porque el código asume "1 factura = 1 asiento" y ese invariante se sostenía solo porque `reverseForSales` borraba. `regenerateForSales` (y su espejo en compras) resolvían "el asiento de esta factura" con `ORDER BY created_at LIMIT 1` **sin filtrar estado** → habrían reescrito el asiento MUERTO. No hay UNIQUE ni FK sobre `(source_type, source_id)`: la BD no avisa. Arreglado con `AND status <> 'VOIDED'` + DESC. Invariante nuevo: **1 factura = como mucho 1 asiento NO anulado**.
>
> **Cómo se montó el e2e** (reutilizable): BD sandbox `benjagest_v8test` en el 3307 (el usuario `benjagest` YA tiene grants sobre `benjagest_v4test`..`v8test`), backend en 8081 con `BENJAGEST_DB_URL`+`BENJAGEST_BACKEND_PORT`. Flyway monta las 176 migraciones de cero. **OJO: no hay usuarios demo** — `V148__purge_demo_data.sql` borra los `%.local`, así que hay que darse de alta por `/api/auth/register` y marcar `email_verified=1` a mano (el PIN está hasheado). Las series RECTIFYING/PROFORMA no se crean por API (las hace el sistema al vuelo); la STANDARD sí, con plantilla `{CODE}-{YYYY}-{0000}` (**`{NNNN}` NO existe** — se guarda literal y luego choca la UK del número). Suite backend: **142 verde**.
>
> ⚠️ **PENDIENTE — smoke VISUAL de Benjamin** (no se puede pulsar botones en JavaFX desde la sesión): dashboard = 1; cobrar → baja solo; Diario → Anular; Diario → Reclasificar cuenta; doble clic en asiento validado → read-only.
> 📌 **Nota nueva**: `nextPostedEntryNumber` quedó idéntico a `nextEntryNumber` en ManualJournalEntryService (dos privados iguales). Unificar cuando se toque.
> 📌 **CLAUDE.md 11.4 desactualizado**: dice "3306 = Pablo, 3307 = proyecto". En la máquina de Benjamin (2026-07-15) escuchan **3306** (MariaDB 12.2, BD casi vacía con F-2026-0001..0003 — la trampa que avisa el doc), **3307** (11.4, la dev, con FRA-2026-*) y **13307** (embebida, la REAL del instalable). La app instalada usa la 13307.
>
> **Sesión 2026-07-15 (noche) — LIQ (asientos de los modelos AEAT) + v0.1.42 + UPD-3 sin mergear.**
>
> - `[x]` **LIQ-303 + V178 + LIQ-BACKFILL + LIQ-UI** (merges `ef963d63`, `1f8a73cd`) — **lo encontró Benjamin mirando su cuenta 477: no cuadraba.** Y se quedó corto: no faltaba un asiento, **faltaba el puente entero fiscal→contable**. El módulo de modelos LEÍA la contabilidad para calcular casillas y **NUNCA le escribía**; al pasar a PAGADO solo hacía `UPDATE tax_filings SET status, csv_aeat, notes`. En todo el proyecto no existía **ni un solo INSERT de asiento fiscal**. Por eso la 477 acumulaba el año entero. **No era una regresión: era algo que nunca se hizo, y que no estaba ni en el backlog.**
>   - **303 al PRESENTAR** → `Debe 477 / Haber 472 / Haber 4750` (o `Debe 4700` si sale a devolver), fechado el último día del periodo. **Es el que vacía la 477.** Al PAGAR → `Debe 4750 / Haber 572`. **130 al PAGAR** → `Debe 473 / Haber 572` (sin liquidación previa: no hay nada acumulado que cancelar).
>   - **V178** — las cuentas **4750/4700 NO EXISTÍAN**: la plantilla permanente (V147) solo llega a 3 dígitos en el grupo 47 (tiene 470 y 475 genéricas). El asiento habría sido imposible de hacer. Aditiva + idempotente (patrón V173); verificado que una empresa que ya las tenía del seed V46 **conservó sus nombres**.
>   - **LIQ-BACKFILL con vista previa**: Benjamin pidió *"un script para que se actualicen mis datos de la db y en el release se actualicen"* y se le propuso lo contrario (y lo aceptó): un script así es una migración Flyway = se ejecuta **sola, en silencio, sobre su contabilidad real**, y escrita a ciegas porque su BD está blindada con DB-LOCK y no se puede auditar antes. La ESTRUCTURA va por migración (V178, no toca datos); los DATOS los repara él desde la app, viéndolos antes. **El orden importa**: la liquidación del 2T solo sale bien si la del 1T ya existe (cada una deja la 477 a cero y la siguiente solo ve lo suyo) → `applyBackfill` reordena por (año, trimestre) pase lo que pase.
>   - **Pregunta MAL PLANTEADA por mí**: le pregunté si su asesoría ya tenía contabilizados el 1T/2T y, al decir que sí, casi cerramos con "no rellenar". **Error mío**: lo que tenga la asesoría está en SU programa, no en los libros de BENJAGEST — son contabilidades distintas y no hay duplicidad posible. Y hay consecuencia real: sin rellenar hacia atrás, el siguiente 303 se llevaría el IVA de TODO el año a la 4750 (el saldo se calcula desde el 1 de enero). Se detectó revisando su respuesta contra el propio código.
>   - **VERIFICADO e2e** (sandbox, no sus datos): 477 → 0,00 tras presentar y la deuda en la 4750; PAID mandado ×3 → **no duplica**; caso a devolver (472 > 477) → usa la **4700** al debe; backfill del 1T → asiento fechado **31/03** (su trimestre, no hoy) y vista previa vacía después.
> - `[~]` **UPD-3** (`1d2915fa`, **en feat/Benjamin, SIN MERGEAR**) — ver el prompt de arranque arriba. **Lo primero de mañana.**
>
> **Gotchas nuevos (costaron tiempo):**
> - **`vatPercent`, NO `vatRate`** en una línea de factura. Con el nombre malo la factura se crea **SIN IVA en silencio** — se habría "verificado" el asiento del 303 contra una 477 vacía y dado por bueno algo falso.
> - En Java, **una barra invertida seguida de "u" es un escape unicode incluso dentro de un comentario**: esa ruta de logs en un javadoc **no compila**. Y el comentario que lo explicaba tampoco compilaba. (También muerde en scripts de Python que generen ese texto.)
> - El PUT de `/tax/filings` exige el objeto completo (taxModelCode/periodYear/periodQuarter), no solo el status.
> - Ruta del 303: `/api/aeat/extras/303/{year}/{quarter}/generate` (no `/api/aeat/303`). Exige el módulo `tax` activo + cabecera `X-Company-Id`.
> - **`build-msi.ps1` MIENTE**: su mensaje final imprime `BENJAGEST Puesto-0.1.35.msi`. El bueno es `dist\msi\BENJAGEST-<version>.msi`. No fiarse del mensaje; mirar el fichero.
> - Inspeccionar un MSI sin instalarlo: `msiexec /a <msi> /qn TARGETDIR=<dir>` (**no** necesita admin).
> - **El registro de Windows MIENTE sobre la versión instalada**: decía 0.1.42 mientras el jar del disco era 0.1.41 (reemplazo aplazado a reinicio). **Comprobar siempre el jar, no el registro.**
>
> ⚠️ **El incidente del actualizador (leer ANTES de tocarlo):**
> 1. La 0.1.39 dejó la app sin arrancar ni como administrador (`runtime\lib\modules` desaparecido → "Failed setting boot class path"). Evidencia del visor de eventos: MsiInstaller **11708 "Installation failed"** + **1038 "Motivo del reinicio: 1" = FICHEROS EN USO**.
> 2. Se arregló reinstalando el MSI a mano. **El MSI estaba SANO** (verificado extrayéndolo con `msiexec /a`: llevaba su modules de 119 MB y su java arrancaba).
> 3. UPD-1/UPD-2 **no bastaron**: perseguían al servicio. La causa real (`PendingFileRenameOperations`) era **la propia app** con sus 39 jar de `app\`.
> 4. **La cola tenía 53 borrados y CERO reemplazos** → reiniciar habría **BORRADO** la app. **Yo había recomendado reiniciar**: era peligroso. Se limpió con `scripts/limpiar-cola-borrado-benjagest.ps1`.
> 5. Al instalar la 0.1.42 encima, el instalador solo ofrecía **Reparar/Eliminar** porque el registro ya decía 0.1.42 (modo mantenimiento). Se resolvió instalando la **0.1.41** (registro y ficheros vuelven a cuadrar) y luego la 0.1.42.
>
> 📌 **Nota de proceso**: parte de la tarde se commiteó **directamente sobre `develop`** (tras el `checkout develop` para sacar el SHA de la release no se volvió a `feat/Benjamin`). Resincronizado con `merge --ff-only`, pero incumple la sección 3 del CLAUDE.md.

> **Sesión 2026-07-15 (tarde) — releases 0.1.39/0.1.40/0.1.41. LAUNCH-1 + UPD-1 + UPD-2 + CRED-OUT.**
>
> ✅ **SMOKE DE BENJAMIN VERDE**: "he podido anular el asiento, y cambiar la cuenta de la venta, todo perfecto". DASH/ASI/ANUL confirmados en la app real.
>
> - 🔥 **La 0.1.39 rompió la instalación de Benjamin al actualizar** ("no arranca el programa aunque lo ejecute como administrador"). El runtime quedó sin `runtime\lib\modules` (119 MB) → `java.exe` moría con "Failed setting boot class path". Evidencia en el visor de eventos: MsiInstaller **11708 "Installation failed"** + **1038 "Motivo del reinicio: 1" = FICHEROS EN USO**. Se arregló reinstalando el MSI a mano (el MSI estaba SANO: se verificó extrayéndolo con `msiexec /a` — llevaba su modules de 119 MB y su java arrancaba).
> - `[x]` **LAUNCH-1** (`45b46a51`) — la app lanzaba un backend HIJO que NO puede abrir la BD del servicio. `isBackendUp()` hacía UNA sonda de 1 s y el servicio tarda ~6 s (levanta MariaDB primero). Si abrías la app en esa ventana → hijo con el usuario de escritorio → `%ProgramData%\BENJAGEST\mariadb-data` es de LocalSystem → "Can't create/write to file '.\ddl_recovery.log' (Errcode: 13 Permission denied)" → Application run failed. Eso era el "levanta el backend pero no levanta la bd"; a la segunda entraba porque el servicio ya respondía. **Fix**: si `sc query BenjagestBackend` da exit 0, JAMÁS se lanza hijo; se espera 90 s. Verificado en ejecución (el hijo no se lanzó: su backend.log quedó intacto).
> - `[x]` **UPD-1** (`e6592256`) — parar el servicio NO bastaba. El servicio corre con **el propio JRE que el MSI reemplaza** (`<executable>%BASE%
untimein\java.exe</executable>`) y no había NINGUNA espera entre `net stop` y `msiexec`: `net stop` vuelve cuando el SCM dice "parado", pero java.exe tarda un instante más en soltar `modules`. **Fix**: el `.cmd` pasa a `.ps1` que pide el PID del servicio ANTES, espera con `Wait-Process` a que MUERA, espera a que nadie corra desde la carpeta, colchón fijo, `msiexec /qb /norestart`, detecta el **3010**, y **VERIFICA que `runtime\lib\modules` exista** al terminar.
> - `[x]` **UPD-2** (`492e396e`, pedido de Benjamin) — ventana de progreso MODAL con barra real y MB (antes: aceptabas y nada visible durante minutos con la app usable, hasta cerrarse sola) + instalador con `-WindowStyle Hidden` para no cerrarlo por error. `download()` pasa de `ofFile` (sin progreso) a copiar el stream contando bytes. **Read-Host → MessageBox**: en una consola oculta, un Read-Host habría esperado para siempre sin que nadie lo viera.
> - `[x]` **CRED-OUT** (`1f8a73cd`) — fuera la pestaña Credenciales. `external_credentials` era un **almacén pasivo con CERO consumidores** (encrypt al guardar, `decrypt` NUNCA). Ver [[project_benjagest_dehu_cerrado]]: el conector DEHú que la justificaba **no se hará** — exige ser "Gran destinatario" (**persona jurídica** + volumen), y Benjamin es autónomo. El **log de uso de certificados** SÍ está vivo (`BrowserCertSessionService.recordUsage`, LOPD) y se movió a la pestaña Certificados. NO destructivo: tabla, service y endpoints intactos.
>
> **Gotchas nuevos:**
> - `build-msi.ps1` **miente al final**: imprime "Instalador: ...BENJAGEST Puesto-0.1.35.msi". El MSI bueno es `dist\msi\BENJAGEST-<version>.msi`. No fiarse del mensaje; mirar el fichero.
> - **`nextPostedEntryNumber` (ANUL-2b)**: cualquier numerador que haga `MAX(entry_number)` filtrando `status='POSTED'` revienta la UK en cuanto existe un asiento ANULADO (conserva su número). Contar TODOS los estados; los DRAFT no estorban (número NULL desde V58).
> - Para inspeccionar un MSI sin instalar: `msiexec /a <msi> /qn TARGETDIR=<dir>` (NO necesita admin).
>
> ⚠️ **PENDIENTE**: (1) la **0.1.40 → 0.1.41 es la PRIMERA prueba real del actualizador nuevo** — mirar que diga "Esperando a que termine el proceso del servicio (PID N)", que NO salga el aviso de reinicio, y que `runtime\lib\modules` siga existiendo después. (2) **Descuadre histórico**: las facturas anuladas ANTES de la 0.1.39 tienen su asiento BORRADO → ese -X sigue en los libros de Benjamin (caso conocido: FRA-2026-0006). Hay que listarlas y decidir la reparación; ojo si el trimestre ya se presentó.
>
> 📌 **Nota de proceso**: parte de esta sesión se commiteó **directamente sobre `develop`** (tras el `checkout develop` para sacar el SHA de la release no se volvió a `feat/Benjamin`). No se perdió nada y se resincronizó con `merge --ff-only`, pero es un incumplimiento de la sección 3 del CLAUDE.md.

> **Sesión 2026-07-12/13 (BUGS post-0.1.36 encontrados por Benjamin — arreglados, pendiente release 0.1.37):**
> - 🐛 **DUP asiento venta PDF + doble 303** (Benjamin al probar Tema 2): reimportar el MISMO PDF duplicaba el asiento y el IVA contaba 2 veces. Causa: el dedup de Tema 2 solo miraba `sales_invoices` (nº+cliente), pero el flujo antiguo `SALES_PDF_IMPORT` dejaba asiento SIN factura → no lo veía. Confirmado con datos reales (asientos 113 SALES_PDF_IMPORT + 114 SALES_INVOICE, mismo `source_pdf_sha256`, ambos 477=304,92). **FIX (`0a5ef4f1`)**: dedup adicional por `source_pdf_sha256` en `SalesPdfImportService` → 409 si el mismo PDF ya generó asiento. **Limpieza en BD de Benjamin (con OK)**: borrado el asiento 113 (3 líneas + learning event; sin pagos/vencimientos colgando) → 303 vuelve a 304,92. (Borrado por SQL directo → Benjamin refresca el Diario en la app.)
> - 🐛 **i18n `enum.invoice_type.HISTORICAL` crudo** en la columna Tipo de Facturación (faltaban HISTORICAL y TEST). **FIX (`0a5ef4f1`)**: añadidas ES (Histórica/Prueba) + EN (Historical/Test).
> - 🔧 **FIX update flow (`fd7192d0`)**: `UpdateService.launchInstaller` ahora para el servicio antes de `msiexec` y lo arranca después (una UAC vía .cmd temporal) → sin reinicio, sin JRE roto. OJO: la update A 0.1.37 aún usa el launchInstaller ANTIGUO → parar el servicio a mano esa última vez.
> - 🐛 **PDF-EXTRACT ventas: NIF del cliente vacío + concepto cortado** (`d54390d1`). Diagnóstico con `RealPdfExtractionDiagnosticTest` sobre el PDF real: el NIF SÍ se detecta, pero el import de venta leía `receiverNif` (vacío en ventas); el correcto es `emitterNif` (el extractor excluye el NIF propio → en factura emitida el "emisor" detectado ES el cliente). Fix UI: cliente=emitterNif/supplierName (+ fallback receiver), y **pre-rellenar el campo Concepto** (antes vacío). Fix backend `findConcept`: capturar continuación multilínea de la descripción (hasta totales/junk), reutilizando `conceptCandidate` como corte. Verificado en ejecución (emitterNif=24259998N, concepto completo). Suite 136 verde. **Pendiente**: el NOMBRE del cliente sigue best-effort en layouts a 2 columnas (otro slice).
> - ⚠️ **TODO pendiente de smoke en ejecución** (dedup SHA + i18n + launchInstaller; extracción ya verificada en el extractor) y de un **release 0.1.37**. Los fixes están en feat/Benjamin, compilan, suite 136 verde; NO mergeado a develop aún.
>
> **Sesión 2026-07-12 (INCIDENTE update 0.1.36 — RESUELTO + fix pendiente):**
> - 🚑 **La auto-actualización a 0.1.36 dejó el JRE roto** (Benjamin: "no arranca benjagest"). Causa raíz: `UpdateService.launchInstaller` hace `msiexec /i` **con el servicio corriendo**; el `java` tiene bloqueado `runtime\lib\modules` (125 MB) → el MSI no puede reemplazarlo en caliente → lo aplaza al reinicio ("reboot required") → la 1ª vez la operación diferida quedó a medias y `lib\modules` desapareció → `java.exe` = "Failed setting boot class path". **El MSI es BUENO** (verificado: contiene `lib\modules` al extraerlo con `msiexec /a`); el fallo es el flujo de update en caliente.
> - ✅ **Reparado**: copiado `lib\modules` desde el app-image local (`dist/out/BENJAGEST/runtime`) a `C:\Program Files\BENJAGEST\runtime` (elevado), servicio a Running/Automatic. Flyway aplicó **V177 sobre la BD real** sin errores (la VM ni arrancó antes, así que no hubo cambios a medias). Benjamin reinstaló + reinició 2ª vez → entró bien. **Estado final: sano, 0.1.36, health 200, sobrevive reinicios.**
> - 🔧 **FIX PENDIENTE (update flow) — PRIORIDAD**: `UpdateService.launchInstaller` (ui/service/UpdateService.java:80) debe **parar el servicio BenjagestBackend ANTES** de `msiexec /i` (y arrancarlo después), en un único proceso elevado, para que los ficheros no estén bloqueados → sin reinicio, sin riesgo. OJO: cambio en el camino de auto-actualización → **necesita smoke de un ciclo de update real antes de fiarse** (no verificable sin una release posterior). Workaround manual hasta entonces: parar el servicio antes de instalar.
>
> **Sesión 2026-07-12 (SMOKE end-to-end de Tema 1 + Tema 2 — VERDE):**
> - ✅ **SMOKE PASADO EN EJECUCIÓN** (Benjamin paró el servicio; embebido aislado en `C:/Proyectos/comp-smoke`, home aislado, BD fresca con V177; empresa 33333333 reusada porque `V148__purge_demo_data` deja las cuentas pero borra companies/users). Restaurado: servicio Windows Running/Automatic, health 200, datos reales `%ProgramData%` intactos (smoke 100% aislado), comp-smoke borrado.
>   - **Tema 2**: import PDF venta → `sales_invoices` HISTORICAL/VALIDATED (aparece en Facturación) + **UN** asiento POSTED enlazado SALES_INVOICE que cuadra (1210=1210); IVA 477 del 303 cuenta **1 vez** (210); reimport → **409 dedup**. ✔
>   - **Tema 1**: /suggestions (compensable 300) · /invoices · /execute → asiento POSTED **Debe 4000001(400) 300 / Haber 4300002(430) 300** cuadrado; venta PARTIAL (700 pendiente = la diferencia); compra PAID; vencimientos partidos (venta PAID 300 + PENDING 700); registro ACTIVE · /list · /pdf (1818 B, %PDF válido) · /reverse → contraasiento POSTED invertido (430/400), venta PENDING (0), compra UNPAID, vencimientos restaurados (Σ1000), registro REVERSED; 303 sin tocar. ✔
> - 🐛 **BUG REAL CAZADO POR EL SMOKE** (`449443f8`): `@RequestParam`/`@PathVariable` sin nombre explícito → 500 en /invoices, /pdf, /reverse (el build no emite `-parameters`). Fix: nombres explícitos. Re-verificado 200. **Lección**: los tests puros no cubren el binding de Spring MVC; el smoke sí.
> - Suite backend 136 verde tras el fix.
>
> **Sesión 2026-07-12 (con Benjamin — Tema 2: factura PDF emitida en Facturación):**
> - 🟡 **TEMA 2 HECHO** (commit en feat/Benjamin, smoke pendiente): `SalesPdfImportService` ahora, además del asiento, crea la factura en `sales_invoices` como **HISTORICAL/VALIDATED** (aparece en Facturación) replicando el import del diario — cliente por NIF + dedup nº+cliente + **asiento POSTED enlazado** (source_type=SALES_INVOICE + source_id) para que el **303 cuente 1 sola vez** (nunca vía SalesInvoiceService, línea roja SIF) + línea + pdf_path conservado en el asiento. **Cambio de comportamiento**: la venta importada entra POSTED (histórica final), ya no DRAFT en "Por validar". La UI ya emite TOPIC_SALES → aparece sola, sin cambios de UI. Result intacto. Suite 136 verde. **Smoke pendiente**: importar PDF venta → aparece en Facturación + 303 cuenta 1 vez.
>
> **Sesión 2026-07-12 (con Benjamin — bloque COMP- compensación de facturas, Tema 1):**
> - 🟡 **Bloque COMP- CONSTRUIDO** (5 slices, commits en feat/Benjamin, **NO pusheado aún, smoke pendiente**): compensación (netting) de una venta (430) contra una compra (400) del mismo tercero por NIF, hasta la cantidad concurrente (CC arts. 1195-1202).
>   - **COMP-1** detección RO (`InvoiceCompensationService`, GET /suggestions + /invoices?nif=; crossMatch PURO, 6 tests).
>   - **COMP-2** ejecución (POST /execute): asiento POSTED Debe 400/Haber 430 vía `ManualJournalEntryService.createImportedPosted` (source_type='COMPENSATION', cuentas por NIF con `TerceroAccountResolverService`); saldado por `invoice_due_dates` PENDING FIFO (medio de pago compensación, SIN asiento de tesorería, parte el parcial); ventas proyectan paid_amount, compras paid=TRUE si a cero. **NO toca SIF, NO dispara REFLEJO.** V177 (invoice_compensations + _lines). allocateFifo PURO, 4 tests (cuadre exacto).
>   - **COMP-3** UI: sub-pestaña "Compensación" en grupo Contabilidad (imita buildBanksTab); propuestas + multi-selección + Compensar + listado con justificante PDF y revertir; i18n ES+EN completo; COMPENSATION en filtro Origen del Diario; endpoints planos para no parsear JSON anidado (§6).
>   - **COMP-4** justificante: `InvoiceCompensationPdfGenerator` (acuerdo CC 1195-1202) + /list, /{id}, /{id}/pdf.
>   - **COMP-5** reversión: POST /{id}/reverse → contraasiento POSTED (no borra el original) + facturas a pendiente.
> - 📚 **Investigación legal+contable** (pedida por Benjamin, "hacerlo por ley"): `docs/compensacion-legal.md`. Claves: la "retención" del art. 1196.5 CC es un EMBARGO (no el IRPF); el IRPF NO cambia el asiento (400/430 ya netos, IRPF va a 4751/473 con Hacienda); IVA/303 NO se tocan (medio de pago, no hecho imponible); 347 igual; SIF fuera. Asiento estándar Debe 400/Haber 430 por el menor.
> - ✅ **Decisiones Benjamin**: (1) hacerlo por ley (investigado); (2) disparo por propuesta automática; (3) escenario bancario "por la diferencia" — un movimiento neto concilia el residual, reutiliza la conciliación existente; (4) alcance mismo NIF con VARIAS facturas por lado.
> - ✅ **Verificado**: compila backend+UI; suite backend 136 tests verdes (10 nuevos PUROS). **PENDIENTE smoke end-to-end en ejecución** (asiento cuadrado/303 no duplica/saldado/reversión/PDF): el servicio Windows ocupa 8080/13307 y el puerto MariaDB embebido está hardcodeado (`EmbeddedMariaDbConfig.PORT`), no se puede levantar un 2º embebido en paralelo. **Requiere parar el servicio un momento o que Benjamin lance el backend** → decisión suya antes de mergear/publicar.
>
> **Sesión 2026-07-12 tarde (con Benjamin — F1-WINSVC + F1-INSTVAR):**
> - ✅ **F1-WINSVC** (`3bfa9a3a` + `677ac313`) — el backend ya corre como **SERVICIO de Windows** (winsw / WinSW.NET4, LocalSystem, auto-arranque). **Verificado end-to-end tras REINICIAR Windows en el PC de Benjamin**: servicio RUNNING, arranca solo en el boot (cuenta `BENJAMIN$`, antes del login), lee la BD en `%ProgramData%\BENJAGEST\mariadb-data`, health 200 sin abrir la app.
> - ✅ **Datos machine-wide** — `BenjagestHome.root()` (nuevo): `-Dbenjagest.home` > `%ProgramData%\BENJAGEST` > `~/.benjagest`. BD + `.dbkey` (ACL ahora con SYSTEM) + facturas + backup + pdfs-importados + fotos-fichaje unificados ahí (8 fallbacks). Imita a `MasterKeyResolver` (la master.key ya vivía en `%ProgramData%\BENJAGEST\secret`).
> - ✅ **Migración de los datos reales de Benjamin** — sus 217 MB de `mariadb-data` + `.dbkey` + facturas (544 K) + backup se copiaron una vez a `%ProgramData%` (origen intacto; respaldo zip en `C:\Users\benja\BENJAGEST-datos-backup-20260712-124113.zip`). Verificado ANTES de instalar: arranque contra la BD migrada → Flyway validó 175 migraciones (schema v176), datos reales presentes (2 companies, 99 asientos, 12 facturas, 1 banco).
> - ✅ **Launcher UI** — si el 8080 ya responde (servicio activo) no lanza otro backend; el mismo binario vale con-servicio y sin.
> - 🐛 **Bug propio cazado en ejecución**: el XML de winsw tenía `--app-content` dentro de un comentario (los comentarios XML no admiten `--`) → winsw no parseaba. Lo destapó Benjamin al `install` (yo no podía, sin elevación). Fix + validación con parser XML real. **Lección: XML/scripts que no ejecuto = sin verificar** (como el "parser verde ≠ camino real" del bloque bancario).
> - ✅ **Scripts auto-elevables** (petición de Benjamin): `install-service.ps1`/`uninstall-service.ps1`/`set-server.ps1` saltan el UAC solos si no van como admin; `.cmd` para doble clic. `install-service.ps1` abre además el 8080 en el firewall (Privado/Dominio; la BD 13307 NO se abre).
> - ✅ **F1-INSTVAR** (`fec5f705`) — `build-msi.ps1 -Variant advisory|puesto`. **advisory** (302 MB, probado) = servidor. **puesto** (71 MB, construido y verificado UI-only) = 2º PC de escritorio LAN. **Pendiente**: probar la conexión en un 2º PC real (Benjamin no tiene; se aceptó red estándar como suficiente — binding 0.0.0.0 confirmado + regla firewall creada).
> - 📌 **Aclaración de roles** (Benjamin): el "empleado final" (fichar/nóminas) va por **PWA móvil** (bloque MEMP, sin instalador), NO por el MSI Puesto. Por eso la variante se llama "Puesto" (2º escritorio de la asesoría), no "Empleado".
> - 📦 **RELEASE v0.1.35**. Suite backend verde. Smoke del servicio real hecho (reinicio incluido).
> - ✅ **Cuadre 130 2T RESUELTO**: Benjamin confirmó con la asesoría que el descuadre era de la ASESORÍA (hasta la nómina les salía descuadrada) → **BENJAGEST está bien** (693,36). Cerrado, no era código.
> - 🔎 **VeriFactu — estado real (leído en código)**: el "modo test" YA está cableado (`verifactu_mode = TEST|PROD`; en TEST el envío va a preproducción AEAT `prewww1.aeat.es`). PERO hoy AEAT rechazaría: el `RegistroFactura` que mandamos es formato INTERNO, no el XSD oficial (TODO en `AeatVerifactuClient`). Para probar de verdad en test hace falta **VF3-FINAL**: cert FNMT cargado + dado de alta como SIF, XML al XSD oficial + firma XAdES-EPES, TrustManager real y parseo de respuesta. Sigue legal en NO VERIFACTU. Plazo: sociedades 1-ene-2027 / autónomos 1-jul-2027.
> - 🐛 **FAC-CLIVAL** (`343c0940`, en feat/Benjamin, **PENDIENTE smoke visual + merge**): bug real — se podía emitir factura ordinaria a cliente con datos incompletos (sin NIF/dirección) sin aviso (PDF con campos en blanco). Ahora "Validar y emitir" (no simplificada) exige NIF+nombre+dirección+población+provincia+CP (art. 6 RD 1619/2012); si faltan → aviso + "Completar datos" (modal cliente reutilizado) → al guardar reintenta la emisión. Bloquea hasta completar. i18n ES+EN. Compila; falta que Benjamin lo pruebe en la UI.
> - 📌 **2º PC (MSI Puesto)**: Benjamin NO tiene 2º equipo → prueba LAN aparcada.
>
...
- **Notas para Benjamin (decisiones/pendientes):**
  1. ✅ **RESUELTA 2026-07-15 (ANUL-1, `023cf39f`)** — ~~Duda contable en anulación (año
     ABIERTO): `voidValidated` BORRA el asiento de la original Y la rectificativa genera su
     asiento negativo → el Diario queda en -X en vez de 0. ¿Es lo querido (¿CONTENDO hacía
     esto?) o debería conservarse el asiento original + negativo (neto 0)? Revisar juntos
     con un caso real antes de tocar.~~
     **Respuesta: se CONSERVA (neto 0).** El caso real lo trajo Benjamin (FRA-2026-0006 +
     0006R). La rectificativa YA ES el contraasiento: no había que añadir nada, había que
     dejar de borrar. Se eliminó la llamada a `reverseForSales` (y el método, que no tenía
     más callers). Verificado e2e: factura de 2000 anulada → neto 0,00 en las dos cuentas y
     la factura conserva su asiento. Detalle: para ejercicio LOCKED/CLOSED esto ya se hacía
     BIEN por accidente (el 409 se absorbía y el comentario lo llamaba "el único camino
     legal") — lo era también para el ejercicio abierto.
     Ver también **ANUL-2**: la misma enfermedad estaba en `voidEntry` (asientos manuales),
     donde VOIDED + contraasiento restaba dos veces.
