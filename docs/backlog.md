...
> **Última actualización: 2026-07-13 — ✅ v0.1.38 PUBLICADA + AUTO-UPDATE VALIDADA + main=develop. La auto-actualización 0.1.37→0.1.38 funcionó SOLA por la vía de la app (launchInstaller para el servicio, instala, arranca; sin parar backend a mano, sin reinicio, lib\modules intacto) → el fix del instalador queda validado end-to-end y el incidente 0.1.36 cerrado. v0.1.38 lleva: concepto extraído no se sobrescribe + botón "Copiar texto del PDF" (PDFBox, capa de texto) + BARRIDO i18n de claves crudas en columnas (worklog SUBMITTED, ss_regime ARTISTAS/MAR/AGRARIO, customer_type MANAGED_CLIENT/INTERNAL, 9 module.*). **RAMAS: main reseteada a develop con force-push autorizado → main = estable/producción (0.1.38), develop = integración/default.** ✅ Smoke visual de Benjamin CONFIRMADO ("todo comprobado y funcionando ok"): concepto + botón copiar-texto OK. Bloque COMP- + Tema 2 + fixes post-release todos cerrados. Histórico abajo.**
>
...
>
> *"Lee la cabecera de docs/backlog.md y docs/plan-estructural-fases.md. Estamos en FASE 1 del plan estructural. Última release **v0.1.35** (F1-WINSVC + F1-INSTVAR). El **BACKEND YA CORRE COMO SERVICIO DE WINDOWS** (winsw, LocalSystem, auto-arranque en el boot antes del login), verificado end-to-end tras reinicio en el PC de Benjamin: RUNNING + health 200 + lee la BD en %ProgramData%. **DECISIÓN CLAVE F1-WINSVC**: los datos se movieron del perfil del usuario a **`%ProgramData%\\BENJAGEST`** (machine-wide, como la master.key de cifrado) vía `BenjagestHome.root()` (-Dbenjagest.home > %ProgramData% > ~/.benjagest); la BD de Benjamin (217 MB), su `.dbkey` (ACL ahora con SYSTEM), facturas y backup se MIGRARON a mano una vez (backup en C:\\Users\\benja\\BENJAGEST-datos-backup-*.zip). El **.dbkey** blinda la BD (DB-LOCK); MariaDB (13307) sigue SOLO en localhost. El backend (8080) escucha en 0.0.0.0 y `install-service.ps1` abre el firewall (Privado/Dominio). **F1-INSTVAR**: `build-msi.ps1 -Variant advisory|puesto`; advisory = servidor completo (probado), **puesto** = 2º PC de escritorio LAN solo-UI (MSI construido, `set-server.ps1 -Server 192.168.0.18`) — **PENDIENTE probar la conexión en un 2º PC real** (Benjamin no tiene; se aceptó red estándar como suficiente: binding 0.0.0.0 + regla firewall). OJO: el 'empleado final' (Rubén: fichar/nóminas) NO es este MSI — es la **PWA móvil** del bloque MEMP, sin instalador. **v0.1.36 CONSTRUIDA pero NO PUBLICADA** (MSI advisory+puesto ya en dist/msi; NO publicar hasta meter Tema 2 + Tema 1, decisión de Benjamin de incluir ambos en la misma versión). Ya lleva: **FAC-CLIVAL** — al 'Validar y emitir' una factura ordinaria (no simplificada), si al cliente le falta NIF/nombre/dirección/población/provincia/CP → aviso 'Faltan datos del cliente' + botón 'Completar datos' (modal reutilizado vía `InvoiceEditorScreen.Host.openCustomerEditor`) → al guardar reintenta la emisión; BLOQUEA hasta completar (art. 6 RD 1619/2012). **PENDIENTE smoke visual de Benjamin.** **DOS TEMAS que faltan para poder publicar v0.1.36** (diagnosticados con agentes Explore): **(TEMA 2 — DECIDIDO 'histórica final como el diario')** la factura EMITIDA importada de PDF NO aparece en Facturación → `SalesPdfImportService` (`accounting/importpdf/`) solo hace INSERT en `journal_entries` (DRAFT), NUNCA en `sales_invoices`, y el listado lee solo de `sales_invoices`; el import de diario (`ContendoImportService.importSale`/`insertSalesInvoice` ~553) sí la inserta. FIX = replicar en `SalesPdfImportService`: crear cliente por NIF + dedup nº+cliente + asiento **POSTED enlazado** (`ManualEntryService.createImportedPosted`, `source_type=SALES_INVOICE`+source_id) + `sales_invoices` HISTORICAL/VALIDATED + línea + pdf_path. **OJO 303 doble conteo**: enlazar EXACTO como el diario (source_type SALES_INVOICE) para contar 1 vez; NUNCA vía SalesInvoiceService (línea roja SIF). **Smoke obligatorio**: importar PDF venta → aparece en Facturación + 303 cuenta 1 sola vez. **(TEMA 1 — BLOQUE NUEVO `COMP-`, NO existe hoy)** compensación (netting) de una venta X (me deben) contra una compra Y del mismo NIF (yo debo): hoy la conciliación bancaria es 1 movimiento ↔ 1 factura, no salda 2 de signos opuestos ni casa un movimiento por la diferencia X−Y. Reutilizable: bloque REFLEJO (`CrossInvoiceReflectionService`, casa por NIF + asientos idempotentes). Falta (a) modelo+asiento de compensación (Debe 400/Haber 430 por el menor Y, deja X con pendiente X−Y) y (b) matching bancario de la diferencia neta. **BLOQUEADO por 4 DECISIONES de Benjamin**: ¿asiento 400/430 por Y ok?; disparo = botón "Compensar" (venta X + compra Y mismo NIF)?; ¿cómo indica los 2 escenarios bancarios (cobro completo de X + pago aparte de Y, vs. un solo movimiento por X−Y)?; ¿solo mismo NIF y 1 venta+1 compra o varias? **VF3 (VeriFactu real) EN MARCHA**: VF3-XML hecho (`AeatRegistroAltaXmlBuilder` puro + 4 tests golden, XSD oficial); la HUELLA ya era correcta y el XAdES NO hace falta para VeriFactu (`ds:Signature` 0..1); siguiente VF3-SIF = identidad del SIF (NombreSistemaInformatico/IdSistemaInformatico 2 chars/Versión/NºInstalación + NombreRazon/NIF del PRODUCTOR = Benjamin) — **esperando a que Benjamin decida darse de alta como productor de software SIF en AEAT**; luego VF3-CHAIN (guardar nº+fecha del registro anterior) · VF3-SEND (embeber en el SOAP + TrustManager real + parsear respuesta) · VF3-CERT (cargar el .p12, sesión conjunta con su contraseña → preproducción mode=TEST). Pendientes menores: **cuadre 130 RESUELTO** (era error de la asesoría, BENJAGEST bien); **F1-INSTVAR-PUESTO en 2º PC** cuando Benjamin lo tenga; **F1-XDIARIO** aparcado sin fichero de referencia. Gotchas: X-Company-Id ad9cb9e6-51bd-4556-8c54-a6f1ecc028d5 para la empresa de Benjamin (pin-login entra como la ASESORÍA); **con el servicio activo NO arrancar otro backend embebido** (pelea por 8080/13307 y el lock de %ProgramData%\\BENJAGEST\\mariadb-data); para smoke aislado usar `-Dbenjagest.home=C:/Proyectos/algo-fuera-de-Temp` (ya NO vale -Duser.home para redirigir la BD); login POST /api/auth/login (token en **accessToken**); release = bump UpdateService.APP_VERSION + build-msi.ps1 -Rebuild (WiX NO en PATH → prepend 'C:\\Program Files (x86)\\WiX Toolset v3.14\\bin') + gh release --target SHA_completo. Instalar servicio en una máquina nueva: MSI advisory → doble clic install-service.cmd (se auto-eleva)."*
>
