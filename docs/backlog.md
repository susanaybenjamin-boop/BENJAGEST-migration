...
> **Última actualización: 2026-07-16 (noche, 4) — ✅ REVISIÓN a fondo del módulo FACTURACIÓN antes de la 1ª factura real (Benjamin lo pidió: al validar no hay vuelta atrás, la BD no se corrige). Flujo feliz VERIFICADO en ejecución (crear→validar→número+huella SIF+asiento). 3 redes de seguridad añadidas y verificadas: FAC-1 (plantilla de serie, impide {NNNN}), FAC-2 (pre-check NIF con mensaje claro), FAC-3 (aislar el asiento: su fallo ya no rompe las cadenas legales inmutables). MERGEADO a develop **SIN release** (junto con GESTOR-CLOSE, esperando agrupar). Suite 155 verde. **OJO — flujo de DOS pasos**: validar factura NO postea el asiento (nace DRAFT, hay que validarlo en el Diario, o el IVA no entra en el 303). Última release publicada sigue siendo v0.1.47. Detalle abajo.**
>
