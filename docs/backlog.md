...
>
> **Última revisión:** 2026-06-04 (sesión PDF compras — **PDF-EXTRACT v2** con layout X/Y por span (PDFBox + LayoutCollector), refactor de regex inspirado en CONTENDO calendarParser.v3. **PDF-TEMPLATES** aprendizaje por NIF (UI editable + plantilla por proveedor, llave de búsqueda = NIF detectado por el extractor, valor mostrado = NIF corregido por el usuario — sobrevive a extractores que fallan de forma consistente). **PDF-AMAZON** específico: NIF español prioritario sobre EU VAT cuando coexisten (Amazon ES → W0184081H, no LU20260743), aceptación de "Número del documento" con `del`, detector de totales por *signature* (línea `21% 35,09 € 7,37 €` + `Total <importe único>` cercano en ±10 líneas — independiente de cabecera). **PDF-MULTI**: extractor devuelve `List<ExtractionResult>`, controller siempre devuelve array, UI muestra un diálogo por factura en secuencia (Amazon agrupa varias facturas por PDF). **TC-CFG** tipos de evento de fichaje configurables + **TC-AUDIT** sub-tab auditoría + **EMP-GEO** geolocalización opcional por empleado. **Próximos**: persistir compras (purchase_invoices CRUD) y modal de comparación de importación de calendario laboral estilo CONTENDO).
...
- ⬜ **PDF-PURCHASES-PERSIST** (siguiente paso natural) — el flujo de extracción hoy termina en un diálogo que cierra sin guardar nada. Hay que crear el módulo Compras completo: tabla `purchase_invoices` (id, company_id, supplier_nif, supplier_name, invoice_number, invoice_date, base_amount, vat_percent, vat_amount, total_amount, document_sha256 UNIQUE por company para dedup, original_pdf_path, status DRAFT/POSTED/VOID, created_by, created_at), `PurchaseInvoiceService` con CRUD + post, endpoint `POST /api/purchases/invoices` desde el diálogo, almacenamiento del PDF original (reutilizar `F-STORAGE` de ventas), botón "Guardar" en el diálogo de extracción que persiste y vincula el PDF. Edición posterior. Listado en módulo Compras con filtros (proveedor, fecha, estado). Integración con futuro libro IVA soportado.
