...
| `7c48684` | infra | Slice A2: TenantContext request-scoped + TenantInterceptor (X-Company-Id header) + ModuleAccessService + @RequiresModule + 403 si modulo inactivo. Repository de Issuer y Customer ya leen company_id del TenantContext (no DemoCompany.ID hardcodeado). Fix: CustomerRepository ahora filtra por company_id en findById/findAllActive (era fuga entre empresas pre-existente). |
| `bf511d7` | infra | Slice C1 backend: login email/password con JWT (HS256, access 8h, refresh 30d). V8 con BCrypt para admin@benjagest.local y empresario@benjagest.local (password "Benjamin123456$") + empresa Marcos Construcciones SL. Spring Security + SecurityConfig stateless + JwtAuthenticationFilter + AuthController (/login, /refresh, /me, /switch-company). TenantInterceptor pasa a leer activeCompanyId del JWT (header X-Company-Id queda como override). PIN renombrado a PinAuthController, sigue en /api/auth/pin para fichaje/desbloqueo. |
| (siguiente) | infra | Slice C1 UI: pantalla email+password, AuthSession singleton con Bearer automatico en los 4 ApiClients, selector de empresa cuando hay varias memberships, deriva modo del company_type, titulo del header dice "Asesoria"/"Empresario" segun modo, toggle eliminado (decision: el modo se elige al alta de empresa). Boton Google deshabilitado (Slice C2). |
